...
@Service
public class MstBloodgroupService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstBloodgroupForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstBloodgroup mstBloodgroup = QMstBloodgroup.mstBloodgroup;
        query.select(Projections.bean(MstBloodgroup.class, mstBloodgroup.bloodgroupId, mstBloodgroup.bloodgroupName)).from(mstBloodgroup).where(mstBloodgroup.isActive.eq(true)).where(mstBloodgroup.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstBloodgroup.bloodgroupName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
