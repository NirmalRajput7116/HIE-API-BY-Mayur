...
//    }
    ...
    public List<Tuple> getInvTaxForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvTax invTax = QInvTax.invTax;
        query.select(Projections.bean(InvTax.class, invTax.taxId, invTax.taxName)).from(invTax).where(invTax.isActive.eq(true)).where(invTax.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invTax.taxName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
