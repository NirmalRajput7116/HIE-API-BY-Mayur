...
@Service
public class MradFilmSizeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMradFilmSizeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradFilmSize mradFilmSize = QMradFilmSize.mradFilmSize;
        query.select(Projections.bean(MradFilmSize.class, mradFilmSize.fsId, mradFilmSize.fsName)).from(mradFilmSize).where(mradFilmSize.isActive.eq(true)).where(mradFilmSize.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradFilmSize.fsName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
