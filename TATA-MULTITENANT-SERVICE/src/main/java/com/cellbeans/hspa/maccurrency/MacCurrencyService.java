...
@Service
public class MacCurrencyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacCurrencyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacCurrency macCurrency = QMacCurrency.macCurrency;
        query.select(Projections.bean(MacCurrency.class, macCurrency.currencyId, macCurrency.currencyName)).from(macCurrency).where(macCurrency.isActive.eq(true)).where(macCurrency.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macCurrency.currencyName.contains(globalFilter).or(macCurrency.currencyCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
