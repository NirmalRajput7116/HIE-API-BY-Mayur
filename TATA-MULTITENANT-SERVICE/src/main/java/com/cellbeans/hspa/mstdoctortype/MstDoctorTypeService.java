...
@Service
public class MstDoctorTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDoctorTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDoctorType mstDoctorType = QMstDoctorType.mstDoctorType;
        query.select(Projections.bean(MstDoctorType.class, mstDoctorType.dtId, mstDoctorType.dtName, mstDoctorType.dtCode)).from(mstDoctorType).where(mstDoctorType.isActive.eq(true)).where(mstDoctorType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDoctorType.dtName.contains(globalFilter).or(mstDoctorType.dtCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
