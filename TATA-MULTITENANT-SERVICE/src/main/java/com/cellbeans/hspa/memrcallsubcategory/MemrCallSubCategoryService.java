...
@Service
public class MemrCallSubCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrCallSubCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrCallSubCategory memrCallSubCategory = QMemrCallSubCategory.memrCallSubCategory;
        query.select(Projections.bean(MemrCallSubCategory.class, memrCallSubCategory.cscId, memrCallSubCategory.ccsName)).from(memrCallSubCategory).where(memrCallSubCategory.isActive.eq(true)).where(memrCallSubCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrCallSubCategory.ccsName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
