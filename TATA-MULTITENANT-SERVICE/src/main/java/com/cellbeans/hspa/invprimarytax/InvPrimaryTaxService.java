...
@Service
public class InvPrimaryTaxService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvPrimaryTaxForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvPrimaryTax invPrimaryTax = QInvPrimaryTax.invPrimaryTax;
        query.select(Projections.bean(InvPrimaryTax.class, invPrimaryTax.ptId, invPrimaryTax.ptName)).from(invPrimaryTax).where(invPrimaryTax.isActive.eq(true)).where(invPrimaryTax.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invPrimaryTax.ptName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
