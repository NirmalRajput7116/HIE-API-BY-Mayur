...
@Service
public class MipdBedAmenityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdBedAmenityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdBedAmenity mipdBedAmenity = QMipdBedAmenity.mipdBedAmenity;
        query.select(Projections.bean(MipdBedAmenity.class, mipdBedAmenity.baId, mipdBedAmenity.baName)).from(mipdBedAmenity).where(mipdBedAmenity.isActive.eq(true)).where(mipdBedAmenity.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdBedAmenity.baName.contains(globalFilter).or(mipdBedAmenity.baCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
