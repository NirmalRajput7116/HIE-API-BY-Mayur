...
@Service
public class MinvMoleculeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvMoleculeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvMolecule minvMolecule = QMinvMolecule.minvMolecule;
        query.select(Projections.bean(MinvMolecule.class, minvMolecule.moleculeId, minvMolecule.moleculeName)).from(minvMolecule).where(minvMolecule.isActive.eq(true)).where(minvMolecule.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvMolecule.moleculeName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
