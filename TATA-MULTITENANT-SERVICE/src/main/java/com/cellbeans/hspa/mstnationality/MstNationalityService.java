...
@Service
public class MstNationalityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstNationalityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstNationality mstNationality = QMstNationality.mstNationality;
        query.select(Projections.bean(MstNationality.class, mstNationality.nationalityId, mstNationality.nationalityName, mstNationality.nationalityCode)).from(mstNationality).where(mstNationality.isActive.eq(true)).where(mstNationality.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstNationality.nationalityName.contains(globalFilter).or(mstNationality.nationalityCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
