...
//        return list;
//    }
    public List<Tuple> getmipdVitalForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdVital mipdVital = QMipdVital.mipdVital;
        query.select(Projections.bean(MipdVital.class, mipdVital.vitalId, mipdVital.vitalName)).from(mipdVital).where(mipdVital.isActive.eq(true)).where(mipdVital.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdVital.vitalName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
