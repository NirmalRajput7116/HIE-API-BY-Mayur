...
@Service
public class MstCenterWaiverService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstCenterWaiverForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCenterWaiver mstCenterWaiver = QMstCenterWaiver.mstCenterWaiver;
        query.select(Projections.bean(MstCenterWaiver.class, mstCenterWaiver.cwId, mstCenterWaiver.cwName)).from(mstCenterWaiver).where(mstCenterWaiver.isActive.eq(true)).where(mstCenterWaiver.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCenterWaiver.cwName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
