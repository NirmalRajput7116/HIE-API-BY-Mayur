...
@Service
public class MstRelationService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstRelationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstRelation mstRelation = QMstRelation.mstRelation;
        query.select(Projections.bean(MstRelation.class, mstRelation.relationId, mstRelation.relationName, mstRelation.relationCode)).from(mstRelation).where(mstRelation.isActive.eq(true)).where(mstRelation.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstRelation.relationName.contains(globalFilter).or(mstRelation.relationCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
