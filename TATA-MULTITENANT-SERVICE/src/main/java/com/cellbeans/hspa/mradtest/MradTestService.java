...
//        return list;
//    }
    public List<Tuple> getMradTestForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradTest mradTest = QMradTest.mradTest;
        query.select(Projections.bean(MradTest.class, mradTest.testId, mradTest.testName)).from(mradTest).where(mradTest.isActive.eq(true)).where(mradTest.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradTest.testName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
