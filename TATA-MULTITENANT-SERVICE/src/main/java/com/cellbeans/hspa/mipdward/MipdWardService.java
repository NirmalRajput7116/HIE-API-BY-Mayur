...
//        return list;
//    }
    public List<Tuple> getMipdWardForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdWard mipdWard = QMipdWard.mipdWard;
        query.select(Projections.bean(MipdWard.class, mipdWard.wardId, mipdWard.wardName)).from(mipdWard).where(mipdWard.isActive.eq(true)).where(mipdWard.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdWard.wardName.contains(globalFilter).or(mipdWard.wardCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
