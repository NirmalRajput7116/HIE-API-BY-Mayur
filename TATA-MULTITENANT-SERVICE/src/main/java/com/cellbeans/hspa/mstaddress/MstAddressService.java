...
@Service
public class MstAddressService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAddressForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAddress mstAddress = QMstAddress.mstAddress;
        query.select(Projections.bean(MstAddress.class, mstAddress.addressId, mstAddress.addressAddress)).from(mstAddress).where(mstAddress.isActive.eq(true)).where(mstAddress.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAddress.addressAddress.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
