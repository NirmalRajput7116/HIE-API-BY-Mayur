...
@Service
public class MinvDispensingService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvDispensingForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvDispensing minvDispensing = QMinvDispensing.minvDispensing;
        query.select(Projections.bean(MinvDispensing.class, minvDispensing.dispensingId, minvDispensing.dispensingName)).from(minvDispensing).where(minvDispensing.isActive.eq(true)).where(minvDispensing.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvDispensing.dispensingName.contains(globalFilter).or(minvDispensing.dispensingCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
