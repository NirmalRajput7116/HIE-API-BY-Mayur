...
@Service
public class MstPriorityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstPriorityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstPriority mstPriority = QMstPriority.mstPriority;
        query.select(Projections.bean(MstPriority.class, mstPriority.priorityId, mstPriority.priorityName)).from(mstPriority).where(mstPriority.isActive.eq(true)).where(mstPriority.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstPriority.priorityName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
