...
@Service
public class MstReferringEntityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstReferringEntityForDropdown(Integer page, Integer size, long entityType, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstReferringEntity mstReferringEntity = QMstReferringEntity.mstReferringEntity;
        if (entityType == 0) {
            query.select(Projections.bean(MstReferringEntity.class, mstReferringEntity.reId, mstReferringEntity.reName, mstReferringEntity.reCode)).from(mstReferringEntity).where(mstReferringEntity.isActive.eq(true)).where(mstReferringEntity.isDeleted.eq(false));
            if (globalFilter != null && !globalFilter.equals("")) {
                query.where(mstReferringEntity.reName.contains(globalFilter).or(mstReferringEntity.reCode.contains(globalFilter)));
            }
        } else {
            query.select(Projections.bean(MstReferringEntity.class, mstReferringEntity.reId, mstReferringEntity.reName, mstReferringEntity.reCode)).from(mstReferringEntity).where(mstReferringEntity.isActive.eq(true)).where(mstReferringEntity.isDeleted.eq(false)).where(mstReferringEntity.reRetId.retId.eq(entityType));
            if (globalFilter != null && !globalFilter.equals("")) {
                query.where(mstReferringEntity.reName.contains(globalFilter).or(mstReferringEntity.reCode.contains(globalFilter)));
            }
        }
