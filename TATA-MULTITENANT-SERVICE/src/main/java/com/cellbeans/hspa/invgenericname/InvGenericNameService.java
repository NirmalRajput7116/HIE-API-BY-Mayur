...
@Service
public class InvGenericNameService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvGenericNameForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvGenericName invGenericName = QInvGenericName.invGenericName;
        query.select(Projections.bean(InvGenericName.class, invGenericName.genericId, invGenericName.genericName)).from(invGenericName).where(invGenericName.isActive.eq(true)).where(invGenericName.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invGenericName.genericName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
