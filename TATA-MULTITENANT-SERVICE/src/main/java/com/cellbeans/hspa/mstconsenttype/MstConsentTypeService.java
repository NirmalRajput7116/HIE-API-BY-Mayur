...
@Service
public class MstConsentTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstConsentTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstConsentType mstConsentType = QMstConsentType.mstConsentType;
        query.select(Projections.bean(MstConsentType.class, mstConsentType.ctId, mstConsentType.ctName)).from(mstConsentType).where(mstConsentType.isActive.eq(true)).where(mstConsentType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstConsentType.ctName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
