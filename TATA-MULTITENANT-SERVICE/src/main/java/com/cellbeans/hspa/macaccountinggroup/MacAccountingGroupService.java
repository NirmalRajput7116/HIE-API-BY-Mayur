...
@Service
public class MacAccountingGroupService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacAccountingGroupForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacAccountingGroup macAccountingGroup = QMacAccountingGroup.macAccountingGroup;
        query.select(Projections.bean(MacAccountingGroup.class, macAccountingGroup.agId, macAccountingGroup.agName)).from(macAccountingGroup).where(macAccountingGroup.isActive.eq(true)).where(macAccountingGroup.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macAccountingGroup.agName.contains(globalFilter).or(macAccountingGroup.agCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
