...
@Service
public class MstPreOperativeInstructionsService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstPreOperativeInstructionsForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstPreOperativeInstructions mstPreOperativeInstructions = QMstPreOperativeInstructions.mstPreOperativeInstructions;
        query.select(Projections.bean(MstPreOperativeInstructions.class, mstPreOperativeInstructions.poiId, mstPreOperativeInstructions.poiName)).from(mstPreOperativeInstructions).where(mstPreOperativeInstructions.isActive.eq(true)).where(mstPreOperativeInstructions.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstPreOperativeInstructions.poiName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
