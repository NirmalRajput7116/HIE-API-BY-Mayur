...
@Service
public class MbillConcessionTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillConcessionTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillConcessionTemplate mbillConcessionTemplate = QMbillConcessionTemplate.mbillConcessionTemplate;
        query.select(Projections.bean(MbillConcessionTemplate.class, mbillConcessionTemplate.ctId, mbillConcessionTemplate.ctName)).from(mbillConcessionTemplate).where(mbillConcessionTemplate.isActive.eq(true)).where(mbillConcessionTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillConcessionTemplate.ctName.contains(globalFilter).or(mbillConcessionTemplate.ctCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
