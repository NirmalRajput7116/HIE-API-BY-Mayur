...
@Service
public class MstSmsTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSmsTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSmsTemplate mstSmsTemplate = QMstSmsTemplate.mstSmsTemplate;
        query.select(Projections.bean(MstSmsTemplate.class, mstSmsTemplate.stId, mstSmsTemplate.stName)).from(mstSmsTemplate).where(mstSmsTemplate.isActive.eq(true)).where(mstSmsTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSmsTemplate.stName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
