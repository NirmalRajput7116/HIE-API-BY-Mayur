...
@Service
public class MstAnaesthesiaNoteService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAnaesthesiaNoteForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAnaesthesiaNote mstAnaesthesiaNote = QMstAnaesthesiaNote.mstAnaesthesiaNote;
        query.select(Projections.bean(MstAnaesthesiaNote.class, mstAnaesthesiaNote.anId, mstAnaesthesiaNote.anLongDescription)).from(mstAnaesthesiaNote).where(mstAnaesthesiaNote.isActive.eq(true)).where(mstAnaesthesiaNote.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAnaesthesiaNote.anLongDescription.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
