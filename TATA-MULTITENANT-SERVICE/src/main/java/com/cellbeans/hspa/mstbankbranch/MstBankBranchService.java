...
@Service
public class MstBankBranchService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstBankBranchForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstBankBranch mstBankBranch = QMstBankBranch.mstBankBranch;
        query.select(Projections.bean(MstBankBranch.class, mstBankBranch.bbId, mstBankBranch.bbName)).from(mstBankBranch).where(mstBankBranch.isActive.eq(true)).where(mstBankBranch.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstBankBranch.bbName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
