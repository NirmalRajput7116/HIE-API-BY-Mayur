...
@Service
public class MstDietUnitService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDietUnitForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDietUnit mstDietUnit = QMstDietUnit.mstDietUnit;
        query.select(Projections.bean(MstDietUnit.class, mstDietUnit.duId, mstDietUnit.duUnitName)).from(mstDietUnit).where(mstDietUnit.isActive.eq(true)).where(mstDietUnit.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDietUnit.duUnitName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
