...
//    }
    ...
    public List<Tuple> getMipdBedForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdBed mipdBed = QMipdBed.mipdBed;
        query.select(Projections.bean(MipdBed.class, mipdBed.bedBcId, mipdBed.bedName)).from(mipdBed).where(mipdBed.isActive.eq(true)).where(mipdBed.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdBed.bedName.contains(globalFilter).or(mipdBed.bedCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
