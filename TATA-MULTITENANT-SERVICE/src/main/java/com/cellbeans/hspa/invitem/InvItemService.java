...
//    }
    ...
    public List<Tuple> getInvItemForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvItem item = QInvItem.invItem;
        query.select(Projections.bean(InvItem.class, item.itemId, item.itemName)).from(item).where(item.isActive.eq(true)).where(item.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(item.itemName.contains(globalFilter).or(item.itemBrandName.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
