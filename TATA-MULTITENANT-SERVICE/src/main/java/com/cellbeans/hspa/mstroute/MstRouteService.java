...
//    }
    ...
    public List<Tuple> getMstRouteForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstRoute mstRoute = QMstRoute.mstRoute;
        query.select(Projections.bean(MstRoute.class, mstRoute.routeId, mstRoute.routeName)).from(mstRoute).where(mstRoute.isActive.eq(true)).where(mstRoute.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstRoute.routeName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
