...
@Service
public class MemrChiefComplaintService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrChiefComplaintForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrChiefComplaint memrChiefComplaint = QMemrChiefComplaint.memrChiefComplaint;
        query.select(Projections.bean(MemrChiefComplaint.class, memrChiefComplaint.ccId, memrChiefComplaint.ccName)).from(memrChiefComplaint).where(memrChiefComplaint.isActive.eq(true)).where(memrChiefComplaint.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrChiefComplaint.ccName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
