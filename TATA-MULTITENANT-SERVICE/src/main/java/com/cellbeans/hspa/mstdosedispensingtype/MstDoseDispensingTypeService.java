...
@Service
public class MstDoseDispensingTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDoseDispensingTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDoseDispensingType mstDoseDispensingType = QMstDoseDispensingType.mstDoseDispensingType;
        query.select(Projections.bean(MstDoseDispensingType.class, mstDoseDispensingType.ddtId, mstDoseDispensingType.ddtName)).from(mstDoseDispensingType).where(mstDoseDispensingType.isActive.eq(true)).where(mstDoseDispensingType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDoseDispensingType.ddtName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
