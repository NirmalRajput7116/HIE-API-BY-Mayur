...
@Service
public class MstReferringEntityTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstReferringEntityTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstReferringEntityType mstReferringEntityType = QMstReferringEntityType.mstReferringEntityType;
        query.select(Projections.bean(MstReferringEntityType.class, mstReferringEntityType.retId, mstReferringEntityType.retName, mstReferringEntityType.retCode)).from(mstReferringEntityType).where(mstReferringEntityType.isActive.eq(true)).where(mstReferringEntityType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstReferringEntityType.retName.contains(globalFilter).or(mstReferringEntityType.retCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
