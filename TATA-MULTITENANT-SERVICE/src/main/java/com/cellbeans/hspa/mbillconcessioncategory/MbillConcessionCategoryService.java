...
@Service
public class MbillConcessionCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillConcessionCategory mbillConcessionCategory = QMbillConcessionCategory.mbillConcessionCategory;
        query.select(Projections.bean(MbillConcessionCategory.class, mbillConcessionCategory.ccId, mbillConcessionCategory.ccName)).from(mbillConcessionCategory).where(mbillConcessionCategory.isActive.eq(true)).where(mbillConcessionCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillConcessionCategory.ccName.contains(globalFilter).or(mbillConcessionCategory.ccCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
