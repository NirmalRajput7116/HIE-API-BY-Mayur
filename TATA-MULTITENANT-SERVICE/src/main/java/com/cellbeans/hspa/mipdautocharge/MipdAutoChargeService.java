...
@Service
public class MipdAutoChargeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdAutoChargeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdAutoCharge mipdAutoCharge = QMipdAutoCharge.mipdAutoCharge;
        query.select(Projections.bean(MipdAutoCharge.class, mipdAutoCharge.atId, mipdAutoCharge.atName)).from(mipdAutoCharge).where(mipdAutoCharge.isActive.eq(true)).where(mipdAutoCharge.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdAutoCharge.atName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
