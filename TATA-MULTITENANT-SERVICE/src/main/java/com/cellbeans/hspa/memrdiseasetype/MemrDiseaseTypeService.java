...
@Service
public class MemrDiseaseTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrDiseaseTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrDiseaseType memrDiseaseType = QMemrDiseaseType.memrDiseaseType;
        query.select(Projections.bean(MemrDiseaseType.class, memrDiseaseType.dtId, memrDiseaseType.dtName)).from(memrDiseaseType).where(memrDiseaseType.isActive.eq(true)).where(memrDiseaseType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrDiseaseType.dtName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
