...
@Service
public class MacAccountHeadService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacAccountHeadForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacAccountHead macAccountHead = QMacAccountHead.macAccountHead;
        query.select(Projections.bean(MacAccountHead.class, macAccountHead.ahId, macAccountHead.ahName)).from(macAccountHead).where(macAccountHead.isActive.eq(true)).where(macAccountHead.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macAccountHead.ahName.contains(globalFilter).or(macAccountHead.ahCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
