...
@Service
public class MbillTermsPaymentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillTermsPaymentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillTermsPayment mbillTermsPayment = QMbillTermsPayment.mbillTermsPayment;
        query.select(Projections.bean(MbillTermsPayment.class, mbillTermsPayment.tpId, mbillTermsPayment.tpName)).from(mbillTermsPayment).where(mbillTermsPayment.isActive.eq(true)).where(mbillTermsPayment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillTermsPayment.tpName.contains(globalFilter).or(mbillTermsPayment.tpCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
