...
@Service
public class MstEmailTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstEmailTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstEmailTemplate mstEmailTemplate = QMstEmailTemplate.mstEmailTemplate;
        query.select(Projections.bean(MstEmailTemplate.class, mstEmailTemplate.etId, mstEmailTemplate.etName)).from(mstEmailTemplate).where(mstEmailTemplate.isActive.eq(true)).where(mstEmailTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstEmailTemplate.etName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
