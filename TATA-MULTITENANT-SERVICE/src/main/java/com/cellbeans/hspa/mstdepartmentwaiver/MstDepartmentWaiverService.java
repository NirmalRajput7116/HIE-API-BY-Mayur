...
@Service
public class MstDepartmentWaiverService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDepartmentWaiverForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDepartmentWaiver mstDepartmentWaiver = QMstDepartmentWaiver.mstDepartmentWaiver;
        query.select(Projections.bean(MstDepartmentWaiver.class, mstDepartmentWaiver.dwId, mstDepartmentWaiver.dwName)).from(mstDepartmentWaiver).where(mstDepartmentWaiver.isActive.eq(true)).where(mstDepartmentWaiver.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDepartmentWaiver.dwName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
