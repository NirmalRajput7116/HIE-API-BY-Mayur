...
@Service
public class MstOperationStatusService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstOperationStatusForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstOperationStatus mstOperationStatus = QMstOperationStatus.mstOperationStatus;
        query.select(Projections.bean(MstOperationStatus.class, mstOperationStatus.osId, mstOperationStatus.osName)).from(mstOperationStatus).where(mstOperationStatus.isActive.eq(true)).where(mstOperationStatus.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstOperationStatus.osName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
