...
@Service
public class MipdRoomAmenityService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMipdRoomAmenityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdRoomAmenity mipdRoomAmenity = QMipdRoomAmenity.mipdRoomAmenity;
        query.select(Projections.bean(MipdRoomAmenity.class, mipdRoomAmenity.raId, mipdRoomAmenity.raName)).from(mipdRoomAmenity).where(mipdRoomAmenity.isActive.eq(true)).where(mipdRoomAmenity.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdRoomAmenity.raName.contains(globalFilter).or(mipdRoomAmenity.raCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
