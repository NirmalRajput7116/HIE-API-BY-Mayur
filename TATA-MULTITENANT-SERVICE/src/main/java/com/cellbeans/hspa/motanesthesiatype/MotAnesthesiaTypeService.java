...
@Service
public class MotAnesthesiaTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotAnesthesiaTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotAnesthesiaType motAnesthesiaType = QMotAnesthesiaType.motAnesthesiaType;
        query.select(Projections.bean(MotAnesthesiaType.class, motAnesthesiaType.atId, motAnesthesiaType.atName)).from(motAnesthesiaType).where(motAnesthesiaType.isActive.eq(true)).where(motAnesthesiaType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motAnesthesiaType.atName.contains(globalFilter).or(motAnesthesiaType.atCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
