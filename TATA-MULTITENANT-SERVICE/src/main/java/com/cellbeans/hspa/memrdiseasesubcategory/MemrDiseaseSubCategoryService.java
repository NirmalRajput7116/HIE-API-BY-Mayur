...
@Service
public class MemrDiseaseSubCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrDiseaseSubCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrDiseaseSubCategory memrDiseaseSubCategory = QMemrDiseaseSubCategory.memrDiseaseSubCategory;
        query.select(Projections.bean(MemrDiseaseSubCategory.class, memrDiseaseSubCategory.dscId, memrDiseaseSubCategory.dcsName)).from(memrDiseaseSubCategory).where(memrDiseaseSubCategory.isActive.eq(true)).where(memrDiseaseSubCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrDiseaseSubCategory.dcsName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
