...
@Service
public class MstMaritalStatusService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstMaritalStatusForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstMaritalStatus mstMaritalStatus = QMstMaritalStatus.mstMaritalStatus;
        query.select(Projections.bean(MstMaritalStatus.class, mstMaritalStatus.msId, mstMaritalStatus.msName)).from(mstMaritalStatus).where(mstMaritalStatus.isActive.eq(true)).where(mstMaritalStatus.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstMaritalStatus.msName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
