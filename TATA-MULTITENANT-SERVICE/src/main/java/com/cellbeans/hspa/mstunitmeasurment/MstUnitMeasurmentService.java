...
@Service
public class MstUnitMeasurmentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstUnitMeasurmentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstUnitMeasurment mstUnitMeasurment = QMstUnitMeasurment.mstUnitMeasurment;
        query.select(Projections.bean(MstUnitMeasurment.class, mstUnitMeasurment.umId, mstUnitMeasurment.umName)).from(mstUnitMeasurment).where(mstUnitMeasurment.isActive.eq(true)).where(mstUnitMeasurment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstUnitMeasurment.umName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
