...
@Service
public class InvPregnancyClassService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvPregnancyClassForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvPregnancyClass invPregnancyClass = QInvPregnancyClass.invPregnancyClass;
        query.select(Projections.bean(InvPregnancyClass.class, invPregnancyClass.pcId, invPregnancyClass.pcName)).from(invPregnancyClass).where(invPregnancyClass.isActive.eq(true)).where(invPregnancyClass.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invPregnancyClass.pcName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
