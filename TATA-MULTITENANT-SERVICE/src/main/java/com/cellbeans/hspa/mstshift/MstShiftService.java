...
//    }
    ...
    public List<Tuple> getMstShiftForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstShift mstShift = QMstShift.mstShift;
        query.select(Projections.bean(MstShift.class, mstShift.shiftId, mstShift.shiftName)).from(mstShift).where(mstShift.isActive.eq(true)).where(mstShift.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstShift.shiftName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
