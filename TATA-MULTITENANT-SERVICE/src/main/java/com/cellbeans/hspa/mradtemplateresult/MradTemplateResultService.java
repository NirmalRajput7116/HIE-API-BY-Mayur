...
@Service
public class MradTemplateResultService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMradTemplateResultForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradTemplateResult mradTemplateResult = QMradTemplateResult.mradTemplateResult;
        query.select(Projections.bean(MradTemplateResult.class, mradTemplateResult.trId, mradTemplateResult.trName)).from(mradTemplateResult).where(mradTemplateResult.isActive.eq(true)).where(mradTemplateResult.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradTemplateResult.trName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
