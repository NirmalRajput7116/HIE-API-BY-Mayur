...
@Service
public class MinvStorageTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvStorageType minvStorageType = QMinvStorageType.minvStorageType;
        query.select(Projections.bean(MinvStorageType.class, minvStorageType.stId, minvStorageType.stName)).from(minvStorageType).where(minvStorageType.isActive.eq(true)).where(minvStorageType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvStorageType.stName.contains(globalFilter).or(minvStorageType.stCode.contains(globalFilter))
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
