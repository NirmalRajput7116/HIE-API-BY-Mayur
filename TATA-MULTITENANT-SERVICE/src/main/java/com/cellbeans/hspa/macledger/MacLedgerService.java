...
//    }
    ...
    public List<Tuple> getMacLedgerForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacLedger macLedger = QMacLedger.macLedger;
        query.select(Projections.bean(MacLedger.class, macLedger.ledgerId, macLedger.ledgerName)).from(macLedger).where(macLedger.isActive.eq(true)).where(macLedger.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macLedger.ledgerName.contains(globalFilter).or(macLedger.ledgerCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
