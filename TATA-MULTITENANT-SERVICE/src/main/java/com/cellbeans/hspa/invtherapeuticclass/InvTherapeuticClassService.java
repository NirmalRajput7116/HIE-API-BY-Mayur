...
@Service
public class InvTherapeuticClassService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvTherapeuticClassForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvTherapeuticClass invTherapeuticClass = QInvTherapeuticClass.invTherapeuticClass;
        query.select(Projections.bean(InvTherapeuticClass.class, invTherapeuticClass.tcId, invTherapeuticClass.tcName)).from(invTherapeuticClass).where(invTherapeuticClass.isActive.eq(true)).where(invTherapeuticClass.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invTherapeuticClass.tcName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
