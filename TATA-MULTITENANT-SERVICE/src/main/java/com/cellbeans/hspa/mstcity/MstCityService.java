...
//        return list;
//    }
    public List<Tuple> getMstCityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCity mstCity = QMstCity.mstCity;
        query.select(Projections.bean(MstCity.class, mstCity.cityId, mstCity.cityName)).from(mstCity).where(mstCity.isActive.eq(true)).where(mstCity.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCity.cityName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
