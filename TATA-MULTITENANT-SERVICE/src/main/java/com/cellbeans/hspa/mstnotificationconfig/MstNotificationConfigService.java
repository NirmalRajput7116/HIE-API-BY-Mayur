...
@Service
public class MstNotificationConfigService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstNotificationConfigForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstNotificationConfig mstNotificationConfig = QMstNotificationConfig.mstNotificationConfig;
        query.select(Projections.bean(MstNotificationConfig.class, mstNotificationConfig.ncId, mstNotificationConfig.ncName)).from(mstNotificationConfig).where(mstNotificationConfig.isActive.eq(true)).where(mstNotificationConfig.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstNotificationConfig.ncName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
