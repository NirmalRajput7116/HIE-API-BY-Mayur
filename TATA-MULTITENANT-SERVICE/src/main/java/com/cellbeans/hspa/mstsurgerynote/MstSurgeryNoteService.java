...
@Service
public class MstSurgeryNoteService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSurgeryNoteForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSurgeryNote mstSurgeryNote = QMstSurgeryNote.mstSurgeryNote;
        query.select(Projections.bean(MstSurgeryNote.class, mstSurgeryNote.snId, mstSurgeryNote.snShortDescription)).from(mstSurgeryNote).where(mstSurgeryNote.isActive.eq(true)).where(mstSurgeryNote.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSurgeryNote.snShortDescription.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
