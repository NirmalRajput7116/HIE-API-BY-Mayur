...
@Service
public class MipdDischargeTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdDischargeTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdDischargeType mipdDischargeType = QMipdDischargeType.mipdDischargeType;
        query.select(Projections.bean(MipdDischargeType.class, mipdDischargeType.dtId, mipdDischargeType.dtName, mipdDischargeType.dtCode)).from(mipdDischargeType).where(mipdDischargeType.isActive.eq(true)).where(mipdDischargeType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdDischargeType.dtName.contains(globalFilter).or(mipdDischargeType.dtCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
