...
@Service
public class InvItemCompanyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemCompanyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvItemCompany invItemCompany = QInvItemCompany.invItemCompany;
        query.select(Projections.bean(InvItemCompany.class, invItemCompany.icId, invItemCompany.icName)).from(invItemCompany).where(invItemCompany.isActive.eq(true)).where(invItemCompany.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invItemCompany.icName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
