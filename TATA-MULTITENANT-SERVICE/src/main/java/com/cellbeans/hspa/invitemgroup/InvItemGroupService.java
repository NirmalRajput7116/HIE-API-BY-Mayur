...
//    }
    ...
    public List<Tuple> getInvItemGroupForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvItemGroup invItemGroup = QInvItemGroup.invItemGroup;
        query.select(Projections.bean(InvItemGroup.class, invItemGroup.igId, invItemGroup.igName)).from(invItemGroup).where(invItemGroup.isActive.eq(true)).where(invItemGroup.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invItemGroup.igName.contains(globalFilter).or(invItemGroup.igLedgerName.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
