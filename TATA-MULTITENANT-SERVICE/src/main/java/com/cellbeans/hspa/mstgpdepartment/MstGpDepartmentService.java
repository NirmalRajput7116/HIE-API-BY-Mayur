...
@Service
public class MstGpDepartmentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstGpDepartmentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstGpDepartment mstGpDepartment = QMstGpDepartment.mstGpDepartment;
        query.select(Projections.bean(MstGpDepartment.class, mstGpDepartment.gpDepartmentId, mstGpDepartment.gpDepartmentName)).from(mstGpDepartment).where(mstGpDepartment.isActive.eq(true)).where(mstGpDepartment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstGpDepartment.gpDepartmentName.contains(globalFilter)
            );
        }
//        query.limit(size);
//        query.offset((page * size) - size);
