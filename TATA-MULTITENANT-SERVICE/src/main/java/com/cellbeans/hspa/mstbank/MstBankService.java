...
//        return list;
//    }
    public List<Tuple> getMstBankForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstBank mstBank = QMstBank.mstBank;
        query.select(Projections.bean(MstBank.class, mstBank.bankId, mstBank.bankName)).from(mstBank).where(mstBank.isActive.eq(true)).where(mstBank.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstBank.bankName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
