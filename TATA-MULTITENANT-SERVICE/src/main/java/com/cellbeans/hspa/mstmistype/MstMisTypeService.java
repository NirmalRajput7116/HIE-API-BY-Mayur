...
@Service
public class MstMisTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstMisTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstMisType mstMisType = QMstMisType.mstMisType;
        query.select(Projections.bean(MstMisType.class, mstMisType.mtId, mstMisType.mtName)).from(mstMisType).where(mstMisType.isActive.eq(true)).where(mstMisType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstMisType.mtName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
