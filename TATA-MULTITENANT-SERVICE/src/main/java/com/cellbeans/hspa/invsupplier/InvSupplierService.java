...
//    }
    ...
    public List<Tuple> getInvSupplierForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvSupplier invSupplier = QInvSupplier.invSupplier;
        query.select(Projections.bean(InvSupplier.class, invSupplier.supplierId, invSupplier.supplierName)).from(invSupplier).where(invSupplier.isActive.eq(true)).where(invSupplier.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invSupplier.supplierName.contains(globalFilter).or(invSupplier.supplierCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
