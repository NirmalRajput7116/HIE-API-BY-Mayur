...
@Service
public class MstStrengthUnitService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstStrengthUnitForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstStrengthUnit mstStrengthUnit = QMstStrengthUnit.mstStrengthUnit;
        query.select(Projections.bean(MstStrengthUnit.class, mstStrengthUnit.suId, mstStrengthUnit.suName)).from(mstStrengthUnit).where(mstStrengthUnit.isActive.eq(true)).where(mstStrengthUnit.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstStrengthUnit.suName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
