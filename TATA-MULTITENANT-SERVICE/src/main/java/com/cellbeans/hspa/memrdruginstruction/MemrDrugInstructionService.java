...
@Service
public class MemrDrugInstructionService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrDrugInstructionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrDrugInstruction memrDrugInstruction = QMemrDrugInstruction.memrDrugInstruction;
        query.select(Projections.bean(MemrDrugInstruction.class, memrDrugInstruction.diId, memrDrugInstruction.diName)).from(memrDrugInstruction).where(memrDrugInstruction.isActive.eq(true)).where(memrDrugInstruction.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrDrugInstruction.diName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
