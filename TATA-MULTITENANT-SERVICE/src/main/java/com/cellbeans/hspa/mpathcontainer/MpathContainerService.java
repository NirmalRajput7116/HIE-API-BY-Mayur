...
@Service
public class MpathContainerService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathContainerForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathContainer mpathContainer = QMpathContainer.mpathContainer;
        query.select(Projections.bean(MpathContainer.class, mpathContainer.containerId, mpathContainer.containerName)).from(mpathContainer).where(mpathContainer.isActive.eq(true)).where(mpathContainer.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathContainer.containerName.contains(globalFilter).or(mpathContainer.containerColor.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
