...
@Service
public class MemrClinicalFormService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrClinicalFormForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrClinicalForm memrClinicalForm = QMemrClinicalForm.memrClinicalForm;
        query.select(Projections.bean(MemrClinicalForm.class, memrClinicalForm.mcfName)).distinct().from(memrClinicalForm).where(memrClinicalForm.isActive.eq(true)).where(memrClinicalForm.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrClinicalForm.mcfName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
