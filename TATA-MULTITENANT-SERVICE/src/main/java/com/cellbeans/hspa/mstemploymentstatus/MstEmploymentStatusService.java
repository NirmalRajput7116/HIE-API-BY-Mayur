...
@Service
public class MstEmploymentStatusService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstEmploymentStatusForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstEmploymentStatus mstEmploymentStatus = QMstEmploymentStatus.mstEmploymentStatus;
        query.select(Projections.bean(MstEmploymentStatus.class, mstEmploymentStatus.esId, mstEmploymentStatus.esName)).from(mstEmploymentStatus).where(mstEmploymentStatus.isActive.eq(true)).where(mstEmploymentStatus.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstEmploymentStatus.esName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
