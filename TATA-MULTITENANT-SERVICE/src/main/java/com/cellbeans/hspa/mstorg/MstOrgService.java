...
//    }
    ...
    public List<Tuple> getMstOrgForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstOrg mstOrg = QMstOrg.mstOrg;
        query.select(Projections.bean(MstOrg.class, mstOrg.orgId, mstOrg.orgName)).from(mstOrg).where(mstOrg.isActive.eq(true)).where(mstOrg.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstOrg.orgName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
