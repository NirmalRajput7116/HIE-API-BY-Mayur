...
@Service
public class MstEthnicityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstEthnicityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstEthnicity mstEthnicity = QMstEthnicity.mstEthnicity;
        query.select(Projections.bean(MstEthnicity.class, mstEthnicity.ethnicityId, mstEthnicity.ethnicityName)).from(mstEthnicity).where(mstEthnicity.isActive.eq(true)).where(mstEthnicity.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstEthnicity.ethnicityName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
