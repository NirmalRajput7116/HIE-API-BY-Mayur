...
@Service
public class MpathParameterUnitService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathParameterUnitForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathParameterUnit mpathParameterUnit = QMpathParameterUnit.mpathParameterUnit;
        query.select(Projections.bean(MpathParameterUnit.class, mpathParameterUnit.puId, mpathParameterUnit.puName)).from(mpathParameterUnit).where(mpathParameterUnit.isActive.eq(true)).where(mpathParameterUnit.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathParameterUnit.puName.contains(globalFilter).or(mpathParameterUnit.puCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
