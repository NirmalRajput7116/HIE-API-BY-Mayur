...
@Service
public class MstPatientTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstPatientTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstPatientType mstPatientType = QMstPatientType.mstPatientType;
        query.select(Projections.bean(MstPatientType.class, mstPatientType.ptId, mstPatientType.ptName, mstPatientType.ptCode)).from(mstPatientType).where(mstPatientType.isActive.eq(true)).where(mstPatientType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstPatientType.ptName.contains(globalFilter).or(mstPatientType.ptCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
