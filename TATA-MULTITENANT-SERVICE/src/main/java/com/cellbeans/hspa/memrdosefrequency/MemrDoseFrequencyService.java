...
@Service
public class MemrDoseFrequencyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrDoseFrequencyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrDoseFrequency memrDoseFrequency = QMemrDoseFrequency.memrDoseFrequency;
        query.select(Projections.bean(MemrDoseFrequency.class, memrDoseFrequency.dfId, memrDoseFrequency.dfName)).from(memrDoseFrequency).where(memrDoseFrequency.isActive.eq(true)).where(memrDoseFrequency.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrDoseFrequency.dfName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
