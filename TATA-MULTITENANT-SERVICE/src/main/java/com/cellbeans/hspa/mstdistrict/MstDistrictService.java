...
//    }
    ...
    public List<Tuple> getMstDistrictForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDistrict mstDistrict = QMstDistrict.mstDistrict;
        query.select(Projections.bean(MstDistrict.class, mstDistrict.districtId, mstDistrict.districtName)).from(mstDistrict).where(mstDistrict.isActive.eq(true)).where(mstDistrict.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDistrict.districtName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
