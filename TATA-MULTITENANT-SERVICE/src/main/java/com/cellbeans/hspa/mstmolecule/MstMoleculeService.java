...
@Service
public class MstMoleculeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstMoleculeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstMolecule mstMolecule = QMstMolecule.mstMolecule;
        query.select(Projections.bean(MstMolecule.class, mstMolecule.moleculeId, mstMolecule.moleculeName)).from(mstMolecule).where(mstMolecule.isActive.eq(true)).where(mstMolecule.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstMolecule.moleculeName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
