...
@Service
public class MotProcedureChecklistService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotProcedureChecklistForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotProcedureChecklist motProcedureChecklist = QMotProcedureChecklist.motProcedureChecklist;
        query.select(Projections.bean(MotProcedureChecklist.class, motProcedureChecklist.pcId, motProcedureChecklist.pcName)).from(motProcedureChecklist).where(motProcedureChecklist.isActive.eq(true)).where(motProcedureChecklist.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motProcedureChecklist.pcName.contains(globalFilter).or(motProcedureChecklist.pcCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
