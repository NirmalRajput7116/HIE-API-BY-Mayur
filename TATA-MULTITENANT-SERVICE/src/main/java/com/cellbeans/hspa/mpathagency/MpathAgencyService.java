...
@Service
public class MpathAgencyService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathAgencyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathAgency mpathAgency = QMpathAgency.mpathAgency;
        query.select(Projections.bean(MpathAgency.class, mpathAgency.agencyId, mpathAgency.agencyName)).from(mpathAgency).where(mpathAgency.isActive.eq(true)).where(mpathAgency.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathAgency.agencyName.contains(globalFilter).or(mpathAgency.agencyCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
