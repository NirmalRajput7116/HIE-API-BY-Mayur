...
@Service
public class MacCostCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacCostCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacCostCategory macCostCategory = QMacCostCategory.macCostCategory;
        query.select(Projections.bean(MacCostCategory.class, macCostCategory.ccId, macCostCategory.ccName)).from(macCostCategory).where(macCostCategory.isActive.eq(true)).where(macCostCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macCostCategory.ccName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
