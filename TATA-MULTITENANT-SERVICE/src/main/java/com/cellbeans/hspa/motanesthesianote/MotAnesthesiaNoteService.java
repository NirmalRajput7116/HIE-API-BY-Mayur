...
@Service
public class MotAnesthesiaNoteService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotAnesthesiaNoteForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotAnesthesiaNote motAnesthesiaNote = QMotAnesthesiaNote.motAnesthesiaNote;
        query.select(Projections.bean(MotAnesthesiaNote.class, motAnesthesiaNote.anId, motAnesthesiaNote.anName)).from(motAnesthesiaNote).where(motAnesthesiaNote.isActive.eq(true)).where(motAnesthesiaNote.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motAnesthesiaNote.anName.contains(globalFilter).or(motAnesthesiaNote.anShortDescription.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
