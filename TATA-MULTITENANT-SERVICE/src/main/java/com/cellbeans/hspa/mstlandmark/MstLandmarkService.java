...
@Service
public class MstLandmarkService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstLandmarkForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstLandmark mstLandmark = QMstLandmark.mstLandmark;
        query.select(Projections.bean(MstLandmark.class, mstLandmark.landmarkId, mstLandmark.landmarkName)).from(mstLandmark).where(mstLandmark.isActive.eq(true)).where(mstLandmark.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstLandmark.landmarkName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
