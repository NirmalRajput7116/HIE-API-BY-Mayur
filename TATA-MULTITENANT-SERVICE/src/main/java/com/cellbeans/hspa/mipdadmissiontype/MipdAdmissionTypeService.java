...
@Service
public class MipdAdmissionTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdAdmissionTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdAdmissionType mipdAdmissionType = QMipdAdmissionType.mipdAdmissionType;
        query.select(Projections.bean(MipdAdmissionType.class, mipdAdmissionType.atId, mipdAdmissionType.atName)).from(mipdAdmissionType).where(mipdAdmissionType.isActive.eq(true)).where(mipdAdmissionType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdAdmissionType.atName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
