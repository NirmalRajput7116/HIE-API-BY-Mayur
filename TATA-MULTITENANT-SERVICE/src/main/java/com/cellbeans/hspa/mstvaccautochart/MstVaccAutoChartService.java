...
@Service
public class MstVaccAutoChartService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstVaccAutoChartForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstVaccAutoChart mstVaccAutoChart = QMstVaccAutoChart.mstVaccAutoChart;
        query.select(Projections.bean(MstVaccAutoChart.class, mstVaccAutoChart.vacId, mstVaccAutoChart.vacDose)).from(mstVaccAutoChart).where(mstVaccAutoChart.isActive.eq(true)).where(mstVaccAutoChart.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstVaccAutoChart.vacDose.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
