...
@Service
public class NstNursingStationDefinationService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getNstNursingStationDefinationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QNstNursingStationDefination nstNursingStationDefination = QNstNursingStationDefination.nstNursingStationDefination;
        query.select(Projections.bean(NstNursingStationDefination.class, nstNursingStationDefination.nsdId, nstNursingStationDefination.nsdName)).from(nstNursingStationDefination).where(nstNursingStationDefination.isActive.eq(true)).where(nstNursingStationDefination.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(nstNursingStationDefination.nsdName.contains(globalFilter).or(nstNursingStationDefination.nsdCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
