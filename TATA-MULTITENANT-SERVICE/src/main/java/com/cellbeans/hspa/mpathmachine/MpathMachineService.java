...
@Service
public class MpathMachineService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathMachineForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathMachine mpathMachine = QMpathMachine.mpathMachine;
        query.select(Projections.bean(MpathMachine.class, mpathMachine.machineId, mpathMachine.machineName)).from(mpathMachine).where(mpathMachine.isActive.eq(true)).where(mpathMachine.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathMachine.machineName.contains(globalFilter).or(mpathMachine.machineCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
