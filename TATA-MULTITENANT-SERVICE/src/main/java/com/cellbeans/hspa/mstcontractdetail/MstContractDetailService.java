...
@Service
public class MstContractDetailService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstContractDetailForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstContractDetail mstContractDetail = QMstContractDetail.mstContractDetail;
        query.select(Projections.bean(MstContractDetail.class, mstContractDetail.cdId, mstContractDetail.cdName)).from(mstContractDetail).where(mstContractDetail.isActive.eq(true)).where(mstContractDetail.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstContractDetail.cdName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
