...
@Service
public class MotProcedureCategoryService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotProcedureCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotProcedureCategory motProcedureCategory = QMotProcedureCategory.motProcedureCategory;
        query.select(Projections.bean(MotProcedureCategory.class, motProcedureCategory.pcId, motProcedureCategory.pcName)).from(motProcedureCategory).where(motProcedureCategory.isActive.eq(true)).where(motProcedureCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motProcedureCategory.pcName.contains(globalFilter).or(motProcedureCategory.pcCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
