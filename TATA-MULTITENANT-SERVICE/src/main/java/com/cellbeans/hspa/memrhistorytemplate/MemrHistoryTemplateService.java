...
@Service
public class MemrHistoryTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrHistoryTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrHistoryTemplate memrHistoryTemplate = QMemrHistoryTemplate.memrHistoryTemplate;
        query.select(Projections.bean(MemrHistoryTemplate.class, memrHistoryTemplate.htId, memrHistoryTemplate.htName)).from(memrHistoryTemplate).where(memrHistoryTemplate.isActive.eq(true)).where(memrHistoryTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrHistoryTemplate.htName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
