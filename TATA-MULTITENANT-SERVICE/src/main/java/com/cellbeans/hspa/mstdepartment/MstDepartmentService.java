...
//    }
    ...
    public List<Tuple> getMstDepartmentForDropdown(Integer page, Integer size, String globalFilter, MstUnit mstUnit) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDepartment mstDepartment = QMstDepartment.mstDepartment;
        query.select(Projections.bean(MstDepartment.class, mstDepartment.departmentId, mstDepartment.departmentName)).from(mstDepartment).where(mstDepartment.isActive.eq(true)).where(mstDepartment.isMedicaldepartment.eq(true)).where(mstDepartment.departmentUnitId.contains(mstUnit)).where(mstDepartment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDepartment.departmentName.contains(globalFilter)
            );
        }
//        query.limit(size);
//        query.offset((page * size) - size);
