...
//    }
    ...
    public List<Tuple> getInvItemCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvItemCategory invItemCategory = QInvItemCategory.invItemCategory;
        query.select(Projections.bean(InvItemCategory.class, invItemCategory.icId, invItemCategory.icName)).from(invItemCategory).where(invItemCategory.isActive.eq(true)).where(invItemCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invItemCategory.icName.contains(globalFilter).or(invItemCategory.icLedgerName.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
