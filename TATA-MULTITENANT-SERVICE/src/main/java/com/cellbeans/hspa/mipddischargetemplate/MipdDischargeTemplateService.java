...
@Service
public class MipdDischargeTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdDischargeTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdDischargeTemplate mipdDischargeTemplate = QMipdDischargeTemplate.mipdDischargeTemplate;
        query.select(Projections.bean(MipdDischargeTemplate.class, mipdDischargeTemplate.dtId, mipdDischargeTemplate.dtName)).from(mipdDischargeTemplate).where(mipdDischargeTemplate.isActive.eq(true)).where(mipdDischargeTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdDischargeTemplate.dtName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
