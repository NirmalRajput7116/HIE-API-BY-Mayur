...
@Service
public class MpathParameterRangeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathParameterRangeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathParameterRange mpathParameterRange = QMpathParameterRange.mpathParameterRange;
        query.select(Projections.bean(MpathParameterRange.class, mpathParameterRange.prId, mpathParameterRange.prMethod)).from(mpathParameterRange).where(mpathParameterRange.isActive.eq(true)).where(mpathParameterRange.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathParameterRange.prMethod.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
