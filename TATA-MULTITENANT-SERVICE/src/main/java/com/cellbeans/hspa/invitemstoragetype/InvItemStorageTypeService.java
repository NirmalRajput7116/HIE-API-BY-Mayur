...
@Service
public class InvItemStorageTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        QInvItemStorageType invItemStorageType = QInvItemStorageType.invItemStorageType;
        query.select(Projections.bean(InvItemStorageType.class, invItemStorageType.istId, invItemStorageType.istName)).from(invItemStorageType).where(invItemStorageType.isActive.eq(true)).where(invItemStorageType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invItemStorageType.istName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
