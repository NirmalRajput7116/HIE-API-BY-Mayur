...
//    }
    ...
    public List<Tuple> getMacParentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacParent macParent = QMacParent.macParent;
        query.select(Projections.bean(MacParent.class, macParent.parentId, macParent.parentName)).from(macParent).where(macParent.isActive.eq(true)).where(macParent.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macParent.parentName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
