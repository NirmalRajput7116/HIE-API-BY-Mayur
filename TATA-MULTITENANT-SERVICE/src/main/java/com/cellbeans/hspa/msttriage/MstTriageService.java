...
//    }
    ...
    public List<Tuple> getMstTriageForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstTriage mstTriage = QMstTriage.mstTriage;
        query.select(Projections.bean(MstTriage.class, mstTriage.triageId, mstTriage.triageName, mstTriage.triageCode)).from(mstTriage).where(mstTriage.isActive.eq(true)).where(mstTriage.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstTriage.triageName.contains(globalFilter).or(mstTriage.triageCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
