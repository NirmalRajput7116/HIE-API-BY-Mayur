...
@Service
public class MstUserTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstUserTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstUserType mstUserType = QMstUserType.mstUserType;
        query.select(Projections.bean(MstUserType.class, mstUserType.utId, mstUserType.utName)).from(mstUserType).where(mstUserType.isActive.eq(true)).where(mstUserType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstUserType.utName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
