...
//        return list;
//    }
    public List<Tuple> getMstCompanyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCompany mstCompany = QMstCompany.mstCompany;
        query.select(Projections.bean(MstCompany.class, mstCompany.companyId, mstCompany.companyName)).from(mstCompany).where(mstCompany.isActive.eq(true)).where(mstCompany.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCompany.companyName.contains(globalFilter).or(mstCompany.companyCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
