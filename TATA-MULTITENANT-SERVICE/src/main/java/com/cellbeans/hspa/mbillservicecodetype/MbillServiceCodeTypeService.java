...
@Service
public class MbillServiceCodeTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillServiceCodeTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillServiceCodeType mbillServiceCodeType = QMbillServiceCodeType.mbillServiceCodeType;
        query.select(Projections.bean(MbillServiceCodeType.class, mbillServiceCodeType.sctId, mbillServiceCodeType.sctName)).from(mbillServiceCodeType).where(mbillServiceCodeType.isActive.eq(true)).where(mbillServiceCodeType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillServiceCodeType.sctName.contains(globalFilter).or(mbillServiceCodeType.sctCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
