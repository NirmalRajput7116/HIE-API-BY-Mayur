...
@Service
public class MpathAgeRangeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathAgeRangeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathAgeRange mpathAgeRange = QMpathAgeRange.mpathAgeRange;
        query.select(Projections.bean(MpathAgeRange.class, mpathAgeRange.arId, mpathAgeRange.arRemark)).from(mpathAgeRange).where(mpathAgeRange.isActive.eq(true)).where(mpathAgeRange.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathAgeRange.arRemark.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
