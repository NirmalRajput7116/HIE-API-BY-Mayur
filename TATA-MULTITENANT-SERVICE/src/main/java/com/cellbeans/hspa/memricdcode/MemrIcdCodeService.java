...
@Service
public class MemrIcdCodeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrIcdCodeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrIcdCode memrIcdCode = QMemrIcdCode.memrIcdCode;
        query.select(Projections.bean(MemrIcdCode.class, memrIcdCode.icId, memrIcdCode.icDescription, memrIcdCode.icCode)).from(memrIcdCode).where(memrIcdCode.isActive.eq(true)).where(memrIcdCode.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrIcdCode.icDescription.contains(globalFilter).or(memrIcdCode.icCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
