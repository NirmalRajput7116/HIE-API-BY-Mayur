...
@Service
public class MstDietPatientCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDietPatientCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDietPatientCategory mstDietPatientCategory = QMstDietPatientCategory.mstDietPatientCategory;
        query.select(Projections.bean(MstDietPatientCategory.class, mstDietPatientCategory.dpcId, mstDietPatientCategory.dpcPatientType)).from(mstDietPatientCategory).where(mstDietPatientCategory.isActive.eq(true)).where(mstDietPatientCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDietPatientCategory.dpcPatientType.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
