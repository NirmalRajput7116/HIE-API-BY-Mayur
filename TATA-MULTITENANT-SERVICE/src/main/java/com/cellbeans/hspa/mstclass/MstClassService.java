...
//        return list;
//    }
    public List<Tuple> getMstClassForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstClass mstClass = QMstClass.mstClass;
        query.select(Projections.bean(MstClass.class, mstClass.classId, mstClass.className)).from(mstClass).where(mstClass.isActive.eq(true)).where(mstClass.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstClass.className.contains(globalFilter).or(mstClass.classCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
