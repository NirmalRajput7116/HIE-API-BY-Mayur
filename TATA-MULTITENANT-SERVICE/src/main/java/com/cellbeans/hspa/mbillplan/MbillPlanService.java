...
//    }
    ...
    public List<Tuple> getMbillPlanForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillPlan mbillPlan = QMbillPlan.mbillPlan;
        query.select(Projections.bean(MbillPlan.class, mbillPlan.planId, mbillPlan.planName)).from(mbillPlan).where(mbillPlan.isActive.eq(true)).where(mbillPlan.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillPlan.planName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
