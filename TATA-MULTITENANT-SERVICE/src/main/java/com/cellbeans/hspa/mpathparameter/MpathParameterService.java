...
@Service
public class MpathParameterService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathParameterForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathParameter mpathParameter = QMpathParameter.mpathParameter;
        query.select(Projections.bean(MpathParameter.class, mpathParameter.parameterId, mpathParameter.parameterName)).from(mpathParameter).where(mpathParameter.isActive.eq(true)).where(mpathParameter.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathParameter.parameterName.contains(globalFilter).or(mpathParameter.parameterCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
