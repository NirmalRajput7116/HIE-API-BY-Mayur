...
@Service
public class MemrAllergySensitivityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrAllergySensitivityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrAllergySensitivity memrAllergySensitivity = QMemrAllergySensitivity.memrAllergySensitivity;
        query.select(Projections.bean(MemrAllergySensitivity.class, memrAllergySensitivity.asId, memrAllergySensitivity.asName)).from(memrAllergySensitivity).where(memrAllergySensitivity.isActive.eq(true)).where(memrAllergySensitivity.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrAllergySensitivity.asName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
