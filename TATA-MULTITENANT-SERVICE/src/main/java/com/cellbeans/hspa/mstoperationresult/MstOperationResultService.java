...
@Service
public class MstOperationResultService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstOperationResultForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstOperationResult mstOperationResult = QMstOperationResult.mstOperationResult;
        query.select(Projections.bean(MstOperationResult.class, mstOperationResult.orId, mstOperationResult.orName)).from(mstOperationResult).where(mstOperationResult.isActive.eq(true)).where(mstOperationResult.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstOperationResult.orName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
