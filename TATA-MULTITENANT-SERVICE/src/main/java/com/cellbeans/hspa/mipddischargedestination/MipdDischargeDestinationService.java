...
@Service
public class
MipdDischargeDestinationService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdDischargeDestinationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdDischargeDestination mipdDischargeDestination = QMipdDischargeDestination.mipdDischargeDestination;
        query.select(Projections.bean(MipdDischargeDestination.class, mipdDischargeDestination.ddId, mipdDischargeDestination.ddName)).from(mipdDischargeDestination).where(mipdDischargeDestination.isActive.eq(true)).where(mipdDischargeDestination.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdDischargeDestination.ddName.contains(globalFilter).or(mipdDischargeDestination.ddCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
