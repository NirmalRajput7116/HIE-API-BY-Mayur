...
@Service
public class MemrClinicalProcedureService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrClinicalProcedureForDropdown(Integer page, Integer size, long specialityId, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrClinicalProcedure memrClinicalProcedure = QMemrClinicalProcedure.memrClinicalProcedure;
        query.select(Projections.bean(MemrClinicalProcedure.class, memrClinicalProcedure.cpId, memrClinicalProcedure.cpName, memrClinicalProcedure.cpContent)).from(memrClinicalProcedure).where(memrClinicalProcedure.isActive.eq(true)).where(memrClinicalProcedure.isDeleted.eq(false)).where(memrClinicalProcedure.cpSpeiclityId.specialityId.eq(specialityId));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrClinicalProcedure.cpName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
