...
@Service
public class MotPreOperativeInstructionService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotPreOperativeInstructionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotPreOperativeInstruction motPreOperativeInstruction = QMotPreOperativeInstruction.motPreOperativeInstruction;
        query.select(Projections.bean(MotPreOperativeInstruction.class, motPreOperativeInstruction.poiId, motPreOperativeInstruction.poiName)).from(motPreOperativeInstruction).where(motPreOperativeInstruction.isActive.eq(true)).where(motPreOperativeInstruction.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motPreOperativeInstruction.poiName.contains(globalFilter).or(motPreOperativeInstruction.poiCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
