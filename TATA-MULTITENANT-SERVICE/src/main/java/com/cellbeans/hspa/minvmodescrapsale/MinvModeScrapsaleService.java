...
@Service
public class MinvModeScrapsaleService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvModeScrapsaleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvModeScrapsale minvModeScrapsale = QMinvModeScrapsale.minvModeScrapsale;
        query.select(Projections.bean(MinvModeScrapsale.class, minvModeScrapsale.mcId, minvModeScrapsale.mcName)).from(minvModeScrapsale).where(minvModeScrapsale.isActive.eq(true)).where(minvModeScrapsale.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvModeScrapsale.mcName.contains(globalFilter).or(minvModeScrapsale.mcCode.contains(globalFilter))
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
