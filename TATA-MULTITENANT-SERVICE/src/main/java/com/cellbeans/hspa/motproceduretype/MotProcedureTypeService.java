...
@Service
public class MotProcedureTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotProcedureTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotProcedureType motProcedureType = QMotProcedureType.motProcedureType;
        query.select(Projections.bean(MotProcedureType.class, motProcedureType.ptId, motProcedureType.ptName)).from(motProcedureType).where(motProcedureType.isActive.eq(true)).where(motProcedureType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motProcedureType.ptName.contains(globalFilter).or(motProcedureType.ptCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
