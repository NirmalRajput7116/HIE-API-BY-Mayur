...
//        return list;
//    }
    public List<Tuple> getMpathTestForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathTest mpathTest = QMpathTest.mpathTest;
        query.select(Projections.bean(MpathTest.class, mpathTest.testId, mpathTest.testName)).from(mpathTest).where(mpathTest.isActive.eq(true)).where(mpathTest.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathTest.testName.contains(globalFilter).or(mpathTest.testCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
