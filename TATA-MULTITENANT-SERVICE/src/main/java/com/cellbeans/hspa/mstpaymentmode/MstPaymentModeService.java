...
@Service
public class MstPaymentModeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstPaymentModeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstPaymentMode mstPaymentMode = QMstPaymentMode.mstPaymentMode;
        query.select(Projections.bean(MstPaymentMode.class, mstPaymentMode.pmId, mstPaymentMode.pmName)).from(mstPaymentMode).where(mstPaymentMode.isActive.eq(true)).where(mstPaymentMode.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstPaymentMode.pmName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
