...
@Service
public class MradTemplateService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMradTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradTemplate mradTemplate = QMradTemplate.mradTemplate;
        query.select(Projections.bean(MradTemplate.class, mradTemplate.templateId, mradTemplate.templateName)).from(mradTemplate).where(mradTemplate.isActive.eq(true)).where(mradTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradTemplate.templateName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
