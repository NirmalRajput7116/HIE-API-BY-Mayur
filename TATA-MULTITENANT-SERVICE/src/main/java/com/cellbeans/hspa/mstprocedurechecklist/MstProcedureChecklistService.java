...
@Service
public class MstProcedureChecklistService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstProcedureChecklistForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstProcedureChecklist mstProcedureChecklist = QMstProcedureChecklist.mstProcedureChecklist;
        query.select(Projections.bean(MstProcedureChecklist.class, mstProcedureChecklist.pcId, mstProcedureChecklist.pcName)).from(mstProcedureChecklist).where(mstProcedureChecklist.isActive.eq(true)).where(mstProcedureChecklist.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstProcedureChecklist.pcName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
