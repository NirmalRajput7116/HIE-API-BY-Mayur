...
//    }
    ...
    public List<Tuple> getMstTitleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstTitle mstTitle = QMstTitle.mstTitle;
        query.select(Projections.bean(MstTitle.class, mstTitle.titleId, mstTitle.titleName)).from(mstTitle).where(mstTitle.isActive.eq(true)).where(mstTitle.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstTitle.titleName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
