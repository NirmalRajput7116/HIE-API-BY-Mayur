...
@Service
public class MstAnaesthesiaTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAnaesthesiaTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAnaesthesiaType mstAnaesthesiaType = QMstAnaesthesiaType.mstAnaesthesiaType;
        query.select(Projections.bean(MstAnaesthesiaType.class, mstAnaesthesiaType.atId, mstAnaesthesiaType.atName)).from(mstAnaesthesiaType).where(mstAnaesthesiaType.isActive.eq(true)).where(mstAnaesthesiaType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAnaesthesiaType.atName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
