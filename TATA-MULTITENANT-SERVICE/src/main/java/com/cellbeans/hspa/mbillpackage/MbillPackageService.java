...
@Service
public class MbillPackageService {
    @Autowired
...
//        return list;
//    }
//    public List<Tuple> getMbillPackageForDropdown(Integer page, Integer size, String globalFilter) {
//        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
//        QMbillPackage mbillPackage = QMbillPackage.mbillPackage;
//        query.select(Projections.bean(MbillPackage.class, mbillPackage.packageId, mbillPackage.packagePackageName)).from(mbillPackage).where(mbillPackage.isActive.eq(true)).where(mbillPackage.isDeleted.eq(false));
//        if (globalFilter != null && !globalFilter.equals("")) {
//            query.where(mbillPackage.packagePackageName.contains(globalFilter).or(mbillPackage.packageCode.contains(globalFilter)));
//        }
//        query.limit(size);
//        query.offset((page * size) - size);
