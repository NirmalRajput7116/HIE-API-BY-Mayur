...
//    }
    ...
    public List<Tuple> getMstModuleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstModule mstModule = QMstModule.mstModule;
        query.select(Projections.bean(MstModule.class, mstModule.moduleId, mstModule.moduleName)).from(mstModule).where(mstModule.isActive.eq(true)).where(mstModule.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstModule.moduleName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
