...
@Service
public class MipdBedClassService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdBedClassForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdBedClass mipdBedClass = QMipdBedClass.mipdBedClass;
        query.select(Projections.bean(MipdBedClass.class, mipdBedClass.bcId, mipdBedClass.bcName)).from(mipdBedClass).where(mipdBedClass.isActive.eq(true)).where(mipdBedClass.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdBedClass.bcName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
