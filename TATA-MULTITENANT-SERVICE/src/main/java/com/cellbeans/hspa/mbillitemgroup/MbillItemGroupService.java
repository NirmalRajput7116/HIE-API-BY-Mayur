...
@Service
public class MbillItemGroupService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillItemGroupForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillItemGroup mbillItemGroup = QMbillItemGroup.mbillItemGroup;
        query.select(Projections.bean(MbillItemGroup.class, mbillItemGroup.igId, mbillItemGroup.igName)).from(mbillItemGroup).where(mbillItemGroup.isActive.eq(true)).where(mbillItemGroup.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillItemGroup.igName.contains(globalFilter).or(mbillItemGroup.igCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
