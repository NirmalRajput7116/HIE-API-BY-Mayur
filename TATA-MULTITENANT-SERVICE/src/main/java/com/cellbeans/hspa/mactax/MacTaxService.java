...
//    }
    ...
    public List<Tuple> getMacTaxForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacTax macTax = QMacTax.macTax;
        query.select(Projections.bean(MacTax.class, macTax.taxId, macTax.taxName)).from(macTax).where(macTax.isActive.eq(true)).where(macTax.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macTax.taxName.contains(globalFilter).or(macTax.taxCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
