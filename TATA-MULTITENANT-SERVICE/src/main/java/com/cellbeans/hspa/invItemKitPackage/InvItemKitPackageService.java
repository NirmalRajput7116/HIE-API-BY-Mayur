...
@Service
public class InvItemKitPackageService {
    @Autowired
    ...
    public List<Tuple> getIkpForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        query.select(Projections.bean(InvItemKitPackage.class, invItemKitPackage.ikpId, invItemKitPackage.ikpName)).from(invItemKitPackage).where(invItemKitPackage.isActive.eq(true)).where(invItemKitPackage.isDeleted.eq(false));
