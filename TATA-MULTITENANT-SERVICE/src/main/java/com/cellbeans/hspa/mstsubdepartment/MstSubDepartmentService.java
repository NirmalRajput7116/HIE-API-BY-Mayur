...
@Service
public class MstSubDepartmentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSubDepartmentForDropdown(Integer page, Integer size, String globalFilter, List<Long> departmentList) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSubDepartment mstSubDepartment = QMstSubDepartment.mstSubDepartment;
        query.select(Projections.bean(MstSubDepartment.class, mstSubDepartment.sdId, mstSubDepartment.sdName)).from(mstSubDepartment).where(mstSubDepartment.isActive.eq(true)).where(mstSubDepartment.sdDepartmentId.departmentId.in(departmentList)).where(mstSubDepartment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSubDepartment.sdName.contains(globalFilter));
        }
//        query.limit(size);
//        query.offset((page * size) - size);
    ...
    public List<Tuple> getMstSubDepartmentForDropdown1(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSubDepartment mstSubDepartment = QMstSubDepartment.mstSubDepartment;
        query.select(Projections.bean(MstSubDepartment.class, mstSubDepartment.sdId, mstSubDepartment.sdName)).from(mstSubDepartment).where(mstSubDepartment.isActive.eq(true)).where(mstSubDepartment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSubDepartment.sdName.contains(globalFilter));
        }
        query.limit(size);
