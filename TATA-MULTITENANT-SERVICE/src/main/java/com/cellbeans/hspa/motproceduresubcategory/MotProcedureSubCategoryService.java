...
@Service
public class MotProcedureSubCategoryService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotProcedureSubCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotProcedureSubCategory motProcedureSubCategory = QMotProcedureSubCategory.motProcedureSubCategory;
        query.select(Projections.bean(MotProcedureSubCategory.class, motProcedureSubCategory.pscId, motProcedureSubCategory.pscName)).from(motProcedureSubCategory).where(motProcedureSubCategory.isActive.eq(true)).where(motProcedureSubCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motProcedureSubCategory.pscName.contains(globalFilter).or(motProcedureSubCategory.pscCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
