...
@Service
public class MipdBedReleaseChecklistService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdBedReleaseChecklistForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdBedReleaseChecklist mipdBedReleaseChecklist = QMipdBedReleaseChecklist.mipdBedReleaseChecklist;
        query.select(Projections.bean(MipdBedReleaseChecklist.class, mipdBedReleaseChecklist.brcId, mipdBedReleaseChecklist.brcName)).from(mipdBedReleaseChecklist).where(mipdBedReleaseChecklist.isActive.eq(true)).where(mipdBedReleaseChecklist.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdBedReleaseChecklist.brcName.contains(globalFilter).or(mipdBedReleaseChecklist.brcCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
