...
//    }
    ...
    public List<Tuple> getMstRaceForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstRace mstRace = QMstRace.mstRace;
        query.select(Projections.bean(MstRace.class, mstRace.raceId, mstRace.raceName, mstRace.raceCode)).from(mstRace).where(mstRace.isActive.eq(true)).where(mstRace.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstRace.raceName.contains(globalFilter).or(mstRace.raceCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
