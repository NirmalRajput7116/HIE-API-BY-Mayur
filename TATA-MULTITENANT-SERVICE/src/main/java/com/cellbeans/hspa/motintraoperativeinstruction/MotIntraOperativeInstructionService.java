...
@Service
public class MotIntraOperativeInstructionService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotIntraOperativeInstructionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotIntraOperativeInstruction motIntraOperativeInstruction = QMotIntraOperativeInstruction.motIntraOperativeInstruction;
        query.select(Projections.bean(MotIntraOperativeInstruction.class, motIntraOperativeInstruction.ioiId, motIntraOperativeInstruction.ioiName)).from(motIntraOperativeInstruction).where(motIntraOperativeInstruction.isActive.eq(true)).where(motIntraOperativeInstruction.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motIntraOperativeInstruction.ioiName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
