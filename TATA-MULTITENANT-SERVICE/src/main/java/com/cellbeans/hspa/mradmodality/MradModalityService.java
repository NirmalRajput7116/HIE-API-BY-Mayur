...
@Service
public class MradModalityService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMradModalityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradModality mradModality = QMradModality.mradModality;
        query.select(Projections.bean(MradModality.class, mradModality.modalityId, mradModality.modalityName)).from(mradModality).where(mradModality.isActive.eq(true)).where(mradModality.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradModality.modalityName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
