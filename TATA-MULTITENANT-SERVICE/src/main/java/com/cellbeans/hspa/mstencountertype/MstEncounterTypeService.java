...
@Service
public class MstEncounterTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstEncounterTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstEncounterType mstEncounterType = QMstEncounterType.mstEncounterType;
        query.select(Projections.bean(MstEncounterType.class, mstEncounterType.etId, mstEncounterType.etName)).from(mstEncounterType).where(mstEncounterType.isActive.eq(true)).where(mstEncounterType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstEncounterType.etName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
