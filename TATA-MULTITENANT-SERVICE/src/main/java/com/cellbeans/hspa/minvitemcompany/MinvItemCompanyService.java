...
@Service
public class MinvItemCompanyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvItemCompanyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvItemCompany minvItemCompany = QMinvItemCompany.minvItemCompany;
        query.select(Projections.bean(MinvItemCompany.class, minvItemCompany.icId, minvItemCompany.icName)).from(minvItemCompany).where(minvItemCompany.isActive.eq(true)).where(minvItemCompany.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvItemCompany.icName.contains(globalFilter).or(minvItemCompany.icCode.contains(globalFilter))
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
