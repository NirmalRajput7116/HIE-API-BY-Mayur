...
@Service
public class InvModeScrapsaleService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvModeScrapsaleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvModeScrapsale invModeScrapsale = QInvModeScrapsale.invModeScrapsale;
        query.select(Projections.bean(InvModeScrapsale.class, invModeScrapsale.mcId, invModeScrapsale.mcName)).from(invModeScrapsale).where(invModeScrapsale.isActive.eq(true)).where(invModeScrapsale.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invModeScrapsale.mcName.contains(globalFilter).or(invModeScrapsale.mcCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
