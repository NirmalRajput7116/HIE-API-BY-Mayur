...
@Service
public class MstClassificationService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstClassificationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstClassification mstClassification = QMstClassification.mstClassification;
        query.select(Projections.bean(MstClassification.class, mstClassification.classificationId, mstClassification.classificationName)).from(mstClassification).where(mstClassification.isActive.eq(true)).where(mstClassification.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstClassification.classificationName.contains(globalFilter).or(mstClassification.classificationCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
