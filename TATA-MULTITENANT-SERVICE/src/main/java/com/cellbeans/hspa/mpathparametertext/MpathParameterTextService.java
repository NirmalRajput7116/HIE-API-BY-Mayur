...
@Service
public class MpathParameterTextService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathParameterTextForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathParameterText mpathParameterText = QMpathParameterText.mpathParameterText;
        query.select(Projections.bean(MpathParameterText.class, mpathParameterText.ptId, mpathParameterText.ptName)).from(mpathParameterText).where(mpathParameterText.isActive.eq(true)).where(mpathParameterText.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathParameterText.ptName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
