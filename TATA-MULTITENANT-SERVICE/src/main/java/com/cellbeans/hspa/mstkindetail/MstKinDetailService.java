...
@Service
public class MstKinDetailService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstKinDetailForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstKinDetail mstKinDetail = QMstKinDetail.mstKinDetail;
        query.select(Projections.bean(MstKinDetail.class, mstKinDetail.kdId, mstKinDetail.kdName)).from(mstKinDetail).where(mstKinDetail.isActive.eq(true)).where(mstKinDetail.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstKinDetail.kdName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
