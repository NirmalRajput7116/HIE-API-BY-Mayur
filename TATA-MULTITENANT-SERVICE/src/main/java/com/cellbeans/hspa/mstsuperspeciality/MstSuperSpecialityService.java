...
@Service
public class MstSuperSpecialityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSuperSpecialityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSuperSpeciality mstSuperSpeciality = QMstSuperSpeciality.mstSuperSpeciality;
        query.select(Projections.bean(MstSuperSpeciality.class, mstSuperSpeciality.ssId, mstSuperSpeciality.ssName)).from(mstSuperSpeciality).where(mstSuperSpeciality.isActive.eq(true)).where(mstSuperSpeciality.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSuperSpeciality.ssName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
    ...
    public List<Tuple> getMstSuperSpecialityForDropdownBySpecilityId(Integer page, Integer size, String globalFilter, long specialityId) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSuperSpeciality mstSuperSpeciality = QMstSuperSpeciality.mstSuperSpeciality;
        query.select(Projections.bean(MstSuperSpeciality.class, mstSuperSpeciality.ssId, mstSuperSpeciality.ssName)).from(mstSuperSpeciality).where(mstSuperSpeciality.isActive.eq(true)).where(mstSuperSpeciality.isDeleted.eq(false)).where(mstSuperSpeciality.ssSpecialityId.specialityId.eq(specialityId));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSuperSpeciality.ssName.contains(globalFilter)
            );
        }
