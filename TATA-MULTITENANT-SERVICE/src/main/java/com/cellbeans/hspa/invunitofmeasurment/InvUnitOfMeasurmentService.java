...
@Service
public class InvUnitOfMeasurmentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvUnitOfMeasurmentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvUnitOfMeasurment invUnitOfMeasurment = QInvUnitOfMeasurment.invUnitOfMeasurment;
        query.select(Projections.bean(InvUnitOfMeasurment.class, invUnitOfMeasurment.uomId, invUnitOfMeasurment.uomName)).from(invUnitOfMeasurment).where(invUnitOfMeasurment.isActive.eq(true)).where(invUnitOfMeasurment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invUnitOfMeasurment.uomName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
