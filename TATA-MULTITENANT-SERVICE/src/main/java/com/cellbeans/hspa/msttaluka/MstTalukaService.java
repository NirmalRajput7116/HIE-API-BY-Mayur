...
//    }
    ...
    public List<Tuple> getMstTalukaForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstTaluka mstTaluka = QMstTaluka.mstTaluka;
        query.select(Projections.bean(MstTaluka.class, mstTaluka.talukaId, mstTaluka.talukaName)).from(mstTaluka).where(mstTaluka.isActive.eq(true)).where(mstTaluka.isDelete.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstTaluka.talukaName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
