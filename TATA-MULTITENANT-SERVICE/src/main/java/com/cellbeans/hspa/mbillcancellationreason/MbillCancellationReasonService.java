...
@Service
public class MbillCancellationReasonService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        QMbillCancellationReason mbillCancellationReason = QMbillCancellationReason.mbillCancellationReason;
        query.select(Projections.bean(MbillCancellationReason.class, mbillCancellationReason.crId, mbillCancellationReason.crName)).from(mbillCancellationReason).where(mbillCancellationReason.isActive.eq(true)).where(mbillCancellationReason.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillCancellationReason.crName.contains(globalFilter).or(mbillCancellationReason.crCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
