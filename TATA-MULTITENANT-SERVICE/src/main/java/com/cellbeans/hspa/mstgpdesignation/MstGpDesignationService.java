...
@Service
public class MstGpDesignationService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDesignationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDesignation mstDesignation = QMstDesignation.mstDesignation;
        query.select(Projections.bean(MstDesignation.class, mstDesignation.designationId, mstDesignation.designationName)).from(mstDesignation).where(mstDesignation.isActive.eq(true)).where(mstDesignation.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDesignation.designationName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
