...
@Service
public class MstAppointmentInstructionService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAppointmentInstructionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAppointmentInstruction mstAppointmentInstruction = QMstAppointmentInstruction.mstAppointmentInstruction;
        query.select(Projections.bean(MstAppointmentInstruction.class, mstAppointmentInstruction.aiId, mstAppointmentInstruction.aiName)).from(mstAppointmentInstruction).where(mstAppointmentInstruction.isActive.eq(true)).where(mstAppointmentInstruction.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAppointmentInstruction.aiName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
