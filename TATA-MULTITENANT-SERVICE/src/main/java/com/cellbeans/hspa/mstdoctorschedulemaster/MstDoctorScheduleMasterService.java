...
@Service
public class MstDoctorScheduleMasterService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDoctorScheduleMasterForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDoctorScheduleMaster mstDoctorScheduleMaster = QMstDoctorScheduleMaster.mstDoctorScheduleMaster;
        query.select(Projections.bean(MstDoctorScheduleMaster.class, mstDoctorScheduleMaster.dsmId, mstDoctorScheduleMaster.dsmDayName)).from(mstDoctorScheduleMaster).where(mstDoctorScheduleMaster.isActive.eq(true)).where(mstDoctorScheduleMaster.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDoctorScheduleMaster.dsmDayName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
