...
@Service
public class MstAppointmentReasonService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAppointmentReasonForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAppointmentReason mstAppointmentReason = QMstAppointmentReason.mstAppointmentReason;
        query.select(Projections.bean(MstAppointmentReason.class, mstAppointmentReason.arId, mstAppointmentReason.arName)).from(mstAppointmentReason).where(mstAppointmentReason.isActive.eq(true)).where(mstAppointmentReason.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAppointmentReason.arName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
