...
@Service
public class MstPatientCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstPatientCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstPatientCategory mstPatientCategory = QMstPatientCategory.mstPatientCategory;
        query.select(Projections.bean(MstPatientCategory.class, mstPatientCategory.pcId, mstPatientCategory.pcName)).from(mstPatientCategory).where(mstPatientCategory.isActive.eq(true)).where(mstPatientCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstPatientCategory.pcName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
