...
//    }
    ...
    public List<Tuple> getMstDayForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDay mstDay = QMstDay.mstDay;
        query.select(Projections.bean(MstDay.class, mstDay.dayId, mstDay.dayName)).from(mstDay).where(mstDay.isActive.eq(true)).where(mstDay.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDay.dayName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
