...
@Service
public class MstCashCounterService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstCashCounterForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCashCounter mstCashCounter = QMstCashCounter.mstCashCounter;
        query.select(Projections.bean(MstCashCounter.class, mstCashCounter.ccId, mstCashCounter.ccName)).from(mstCashCounter).where(mstCashCounter.isActive.eq(true)).where(mstCashCounter.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCashCounter.ccName.contains(globalFilter).or(mstCashCounter.ccCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
