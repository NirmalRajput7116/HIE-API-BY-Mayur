...
//    }
    ...
    public List<Tuple> getMstUnitForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstUnit mstUnit = QMstUnit.mstUnit;
        query.select(Projections.bean(MstUnit.class, mstUnit.unitId, mstUnit.unitName,
                mstUnit.unitCityId.cityId, mstUnit.unitCityId.cityName))
                .from(mstUnit)
                .where(mstUnit.isActive.eq(true))
                .where(mstUnit.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstUnit.unitName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
