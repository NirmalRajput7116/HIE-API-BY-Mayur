...
@Service
public class MotOperationResultService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMotOperationResultForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMotOperationResult motOperationResult = QMotOperationResult.motOperationResult;
        query.select(Projections.bean(MotOperationResult.class, motOperationResult.oprId, motOperationResult.oprName)).from(motOperationResult).where(motOperationResult.isActive.eq(true)).where(motOperationResult.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(motOperationResult.oprName.contains(globalFilter).or(motOperationResult.oprCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
