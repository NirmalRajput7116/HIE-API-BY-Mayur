...
@Service
public class MbillItemCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        QMbillItemCategory mbillItemCategory = QMbillItemCategory.mbillItemCategory;
        query.select(Projections.bean(MbillItemCategory.class, mbillItemCategory.icId, mbillItemCategory.icName)).from(mbillItemCategory).where(mbillItemCategory.isActive.eq(true)).where(mbillItemCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillItemCategory.icName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
