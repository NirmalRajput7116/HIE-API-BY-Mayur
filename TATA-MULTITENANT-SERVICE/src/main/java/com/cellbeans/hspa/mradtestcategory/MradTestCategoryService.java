...
@Service
public class MradTestCategoryService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMradTestCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMradTestCategory mradTestCategory = QMradTestCategory.mradTestCategory;
        query.select(Projections.bean(MradTestCategory.class, mradTestCategory.tcId, mradTestCategory.tcName)).from(mradTestCategory).where(mradTestCategory.isActive.eq(true)).where(mradTestCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mradTestCategory.tcName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
