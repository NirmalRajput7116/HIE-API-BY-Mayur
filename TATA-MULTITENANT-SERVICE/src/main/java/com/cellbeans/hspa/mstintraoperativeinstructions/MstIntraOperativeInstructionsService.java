...
@Service
public class MstIntraOperativeInstructionsService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstIntraOperativeInstructionsForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstIntraOperativeInstructions mstIntraOperativeInstructions = QMstIntraOperativeInstructions.mstIntraOperativeInstructions;
        query.select(Projections.bean(MstIntraOperativeInstructions.class, mstIntraOperativeInstructions.ioiId, mstIntraOperativeInstructions.ioiName)).from(mstIntraOperativeInstructions).where(mstIntraOperativeInstructions.isActive.eq(true)).where(mstIntraOperativeInstructions.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstIntraOperativeInstructions.ioiName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
