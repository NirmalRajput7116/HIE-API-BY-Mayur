...
@Service
public class MstMlcJuridictionService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstMlcJuridictionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstMlcJuridiction mstMlcJuridiction = QMstMlcJuridiction.mstMlcJuridiction;
        query.select(Projections.bean(MstMlcJuridiction.class, mstMlcJuridiction.mjId, mstMlcJuridiction.mjName, mstMlcJuridiction.mjCode)).from(mstMlcJuridiction).where(mstMlcJuridiction.isActive.eq(true)).where(mstMlcJuridiction.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstMlcJuridiction.mjName.contains(globalFilter).or(mstMlcJuridiction.mjCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
