...
//    }
    ...
    public List<Tuple> getMstRoleForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        query.select(Projections.bean(MstRole.class, mstRole.roleId, mstRole.roleName)).from(mstRole).where(mstRole.isActive.eq(true)).where(mstRole.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
