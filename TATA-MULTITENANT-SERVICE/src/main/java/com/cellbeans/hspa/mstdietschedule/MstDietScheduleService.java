...
@Service
public class MstDietScheduleService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDietScheduleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDietSchedule mstDietSchedule = QMstDietSchedule.mstDietSchedule;
        query.select(Projections.bean(MstDietSchedule.class, mstDietSchedule.dsId, mstDietSchedule.dsDietName)).from(mstDietSchedule).where(mstDietSchedule.isActive.eq(true)).where(mstDietSchedule.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDietSchedule.dsDietName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
