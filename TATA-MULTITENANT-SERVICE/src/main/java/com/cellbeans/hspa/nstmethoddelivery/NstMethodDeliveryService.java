...
@Service
public class NstMethodDeliveryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getNstMethodDeliveryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QNstMethodDelivery nstMethodDelivery = QNstMethodDelivery.nstMethodDelivery;
        query.select(Projections.bean(NstMethodDelivery.class, nstMethodDelivery.mdId, nstMethodDelivery.mdName)).from(nstMethodDelivery).where(nstMethodDelivery.isActive.eq(true)).where(nstMethodDelivery.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(nstMethodDelivery.mdName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
