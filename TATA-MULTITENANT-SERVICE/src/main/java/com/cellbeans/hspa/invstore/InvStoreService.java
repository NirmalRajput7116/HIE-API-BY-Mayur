...
//    }
    ...
    public List<Tuple> getInvStoreForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvStore invStore = QInvStore.invStore;
        query.select(Projections.bean(InvStore.class, invStore.storeId, invStore.storeName)).from(invStore).where(invStore.isActive.eq(true)).where(invStore.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invStore.storeName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
