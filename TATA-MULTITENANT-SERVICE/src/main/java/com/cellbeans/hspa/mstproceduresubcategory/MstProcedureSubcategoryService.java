...
@Service
public class MstProcedureSubcategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstProcedureSubcategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstProcedureSubcategory mstProcedureSubcategory = QMstProcedureSubcategory.mstProcedureSubcategory;
        query.select(Projections.bean(MstProcedureSubcategory.class, mstProcedureSubcategory.psId, mstProcedureSubcategory.psName)).from(mstProcedureSubcategory).where(mstProcedureSubcategory.isActive.eq(true)).where(mstProcedureSubcategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstProcedureSubcategory.psName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
