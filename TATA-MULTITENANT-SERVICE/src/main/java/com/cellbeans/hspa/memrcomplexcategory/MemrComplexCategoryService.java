...
@Service
public class MemrComplexCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrComplexCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrComplexCategory memrComplexCategory = QMemrComplexCategory.memrComplexCategory;
        query.select(Projections.bean(MemrComplexCategory.class, memrComplexCategory.ccId, memrComplexCategory.ccName)).from(memrComplexCategory).where(memrComplexCategory.isActive.eq(true)).where(memrComplexCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrComplexCategory.ccName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
