...
@Service
public class MacTaxTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacTaxTypeTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacTaxType macTaxType = QMacTaxType.macTaxType;
        query.select(Projections.bean(MacTaxType.class, macTaxType.ttId, macTaxType.ttName)).from(macTaxType).where(macTaxType.isActive.eq(true)).where(macTaxType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macTaxType.ttName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
