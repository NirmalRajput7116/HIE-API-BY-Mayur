...
@Service
public class MstIncomeCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstIncomeCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstIncomeCategory mstIncomeCategory = QMstIncomeCategory.mstIncomeCategory;
        query.select(Projections.bean(MstIncomeCategory.class, mstIncomeCategory.icId, mstIncomeCategory.icName)).from(mstIncomeCategory).where(mstIncomeCategory.isActive.eq(true)).where(mstIncomeCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstIncomeCategory.icName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
