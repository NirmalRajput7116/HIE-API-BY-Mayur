...
@Service
public class MbillSubGroupService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillSubGroupForDropdown(Integer page, Integer size, String groupId, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillSubGroup mbillSubGroup = QMbillSubGroup.mbillSubGroup;
        System.out.println("--------sub group----------------");
        if (groupId.equals(null)) {
            System.out.println("------zero----------");
            query.select(Projections.bean(MbillSubGroup.class, mbillSubGroup.sgId, mbillSubGroup.sgName)).from(mbillSubGroup).where(mbillSubGroup.isActive.eq(true)).where(mbillSubGroup.isDeleted.eq(false));
            if (globalFilter != null && !globalFilter.equals("")) {
                query.where(mbillSubGroup.sgName.contains(globalFilter).or(mbillSubGroup.sgCode.contains(globalFilter)));
            }
        } else {
            System.out.println("-----non zero--------");
            query.select(Projections.bean(MbillSubGroup.class, mbillSubGroup.sgId, mbillSubGroup.sgName)).from(mbillSubGroup).where(mbillSubGroup.isActive.eq(true)).where(mbillSubGroup.isDeleted.eq(false)).where(mbillSubGroup.sgGroupId.groupId.eq(Long.parseLong(groupId)));
            if (globalFilter != null && !globalFilter.equals("")) {
                query.where(mbillSubGroup.sgName.contains(globalFilter).or(mbillSubGroup.sgCode.contains(globalFilter)));
            }
        }
