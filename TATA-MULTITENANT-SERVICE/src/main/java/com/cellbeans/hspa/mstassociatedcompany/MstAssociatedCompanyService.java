...
@Service
public class MstAssociatedCompanyService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAssociatedCompanyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAssociatedCompany mstAssociatedCompany = QMstAssociatedCompany.mstAssociatedCompany;
        query.select(Projections.bean(MstAssociatedCompany.class, mstAssociatedCompany.acId, mstAssociatedCompany.acName)).from(mstAssociatedCompany).where(mstAssociatedCompany.isActive.eq(true)).where(mstAssociatedCompany.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAssociatedCompany.acName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
