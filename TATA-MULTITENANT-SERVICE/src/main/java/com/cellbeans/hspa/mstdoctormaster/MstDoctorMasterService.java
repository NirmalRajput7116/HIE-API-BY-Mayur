...
@Service
public class MstDoctorMasterService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDoctorMasterForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDoctorMaster mstDoctorMaster = QMstDoctorMaster.mstDoctorMaster;
        query.select(Projections.bean(MstDoctorMaster.class, mstDoctorMaster.dmId, mstDoctorMaster.dmFirstName)).from(mstDoctorMaster).where(mstDoctorMaster.isActive.eq(true)).where(mstDoctorMaster.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDoctorMaster.dmFirstName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
