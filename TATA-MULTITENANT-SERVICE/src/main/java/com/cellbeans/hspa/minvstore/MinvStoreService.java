...
//    }
    ...
    public List<Tuple> getMinvStoreForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvStore minvStore = QMinvStore.minvStore;
        query.select(Projections.bean(MinvStore.class, minvStore.storeId, minvStore.storeName)).from(minvStore).where(minvStore.isActive.eq(true)).where(minvStore.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvStore.storeName.contains(globalFilter).or(minvStore.storeCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
