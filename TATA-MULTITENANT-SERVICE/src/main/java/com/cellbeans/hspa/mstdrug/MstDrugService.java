...
//    }
    ...
    public List<Tuple> getMstDrugForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDrug mstDrug = QMstDrug.mstDrug;
        query.select(Projections.bean(MstDrug.class, mstDrug.drugId, mstDrug.drugName, mstDrug.drugCode)).from(mstDrug).where(mstDrug.isActive.eq(true)).where(mstDrug.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDrug.drugName.contains(globalFilter).or(mstDrug.drugCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
