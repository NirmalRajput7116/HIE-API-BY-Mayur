...
@Service
public class MemrAllergyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrAllergyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrAllergy memrAllergy = QMemrAllergy.memrAllergy;
        query.select(Projections.bean(MemrAllergy.class, memrAllergy.allergyId, memrAllergy.allergyName)).from(memrAllergy).where(memrAllergy.isActive.eq(true)).where(memrAllergy.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrAllergy.allergyName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
