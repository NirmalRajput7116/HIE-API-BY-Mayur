...
//    }
    ...
    public List<Tuple> getMstGenderForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstGender mstGender = QMstGender.mstGender;
        query.select(Projections.bean(MstGender.class, mstGender.genderId, mstGender.genderName)).from(mstGender).where(mstGender.isActive.eq(true)).where(mstGender.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstGender.genderName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
