...
@Service
public class MstSpecialityService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSpecialityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSpeciality mstSpeciality = QMstSpeciality.mstSpeciality;
        query.select(Projections.bean(MstSpeciality.class, mstSpeciality.specialityId, mstSpeciality.specialityName)).from(mstSpeciality).where(mstSpeciality.isActive.eq(true)).where(mstSpeciality.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSpeciality.specialityName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
    ...
    public List<Tuple> getAllMstSpecialityForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSpeciality mstSpeciality = QMstSpeciality.mstSpeciality;
        query.select(Projections.bean(MstSpeciality.class, mstSpeciality.specialityId, mstSpeciality.specialityName))
                .from(mstSpeciality)
                .where(mstSpeciality.isActive.eq(true))
                .where(mstSpeciality.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSpeciality.specialityName.contains(globalFilter)
            );
        }
