...
@Service
public class MstDocumentCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDocumentCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDocumentCategory mstDocumentCategory = QMstDocumentCategory.mstDocumentCategory;
        query.select(Projections.bean(MstDocumentCategory.class, mstDocumentCategory.dcId, mstDocumentCategory.dcName, mstDocumentCategory.dcCode)).from(mstDocumentCategory).where(mstDocumentCategory.isActive.eq(true)).where(mstDocumentCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDocumentCategory.dcName.contains(globalFilter).or(mstDocumentCategory.dcCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
