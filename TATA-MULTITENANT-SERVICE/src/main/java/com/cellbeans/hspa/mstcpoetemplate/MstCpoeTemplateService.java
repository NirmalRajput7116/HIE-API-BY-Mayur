...
@Service
public class MstCpoeTemplateService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstCpoeTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCpoeTemplate mstCpoeTemplate = QMstCpoeTemplate.mstCpoeTemplate;
        query.select(Projections.bean(MstCpoeTemplate.class, mstCpoeTemplate.ctId, mstCpoeTemplate.ctName)).from(mstCpoeTemplate).where(mstCpoeTemplate.isActive.eq(true)).where(mstCpoeTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCpoeTemplate.ctName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
