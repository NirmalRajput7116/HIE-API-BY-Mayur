...
@Service
public class MstDoseFrequencyService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDoseFrequencyForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDoseFrequency mstDoseFrequency = QMstDoseFrequency.mstDoseFrequency;
        query.select(Projections.bean(MstDoseFrequency.class, mstDoseFrequency.dfId, mstDoseFrequency.dfName)).from(mstDoseFrequency).where(mstDoseFrequency.isActive.eq(true)).where(mstDoseFrequency.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDoseFrequency.dfName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
