...
@Service
public class InvItemTermsAndConditionService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemTermsAndConditionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvItemTermsAndCondition invItemTermsAndCondition = QInvItemTermsAndCondition.invItemTermsAndCondition;
        query.select(Projections.bean(InvItemTermsAndCondition.class, invItemTermsAndCondition.tacId, invItemTermsAndCondition.tacDescripation)).from(invItemTermsAndCondition).where(invItemTermsAndCondition.isActive.eq(true)).where(invItemTermsAndCondition.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invItemTermsAndCondition.tacDescripation.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
