...
//        return list;
//    }
    public List<Tuple> getMstCabinForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCabin mstCabin = QMstCabin.mstCabin;
        query.select(Projections.bean(MstCabin.class, mstCabin.cabinId, mstCabin.cabinName)).from(mstCabin).where(mstCabin.isActive.eq(true)).where(mstCabin.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCabin.cabinName.contains(globalFilter)
            );
        }
//        query.limit(size);
//        query.offset((page * size) - size);
