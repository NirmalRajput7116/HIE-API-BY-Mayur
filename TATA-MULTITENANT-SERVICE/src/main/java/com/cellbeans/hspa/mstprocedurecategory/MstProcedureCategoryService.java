...
@Service
public class MstProcedureCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstProcedureCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstProcedureCategory mstProcedureCategory = QMstProcedureCategory.mstProcedureCategory;
        query.select(Projections.bean(MstProcedureCategory.class, mstProcedureCategory.pcId, mstProcedureCategory.pcName)).from(mstProcedureCategory).where(mstProcedureCategory.isActive.eq(true)).where(mstProcedureCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstProcedureCategory.pcName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
