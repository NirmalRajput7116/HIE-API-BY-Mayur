...
//    }
    ...
    public List<Tuple> getMipdBlockForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdBlock mipdBlock = QMipdBlock.mipdBlock;
        query.select(Projections.bean(MipdBlock.class, mipdBlock.blockId, mipdBlock.blockName)).from(mipdBlock).where(mipdBlock.isActive.eq(true)).where(mipdBlock.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdBlock.blockName.contains(globalFilter).or(mipdBlock.blockCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
