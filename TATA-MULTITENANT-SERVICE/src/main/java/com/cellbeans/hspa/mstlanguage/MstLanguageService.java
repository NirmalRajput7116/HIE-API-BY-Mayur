...
//    }
    ...
    public List<Tuple> getMstLanguageForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstLanguage mstLanguage = QMstLanguage.mstLanguage;
        query.select(Projections.bean(MstLanguage.class, mstLanguage.languageId, mstLanguage.languageName)).from(mstLanguage).where(mstLanguage.isActive.eq(true)).where(mstLanguage.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstLanguage.languageName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
