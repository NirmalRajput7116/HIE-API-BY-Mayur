...
@Service
public class MstOperationTheatreTableService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstOperationTheatreTableForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstOperationTheatreTable mstOperationTheatreTable = QMstOperationTheatreTable.mstOperationTheatreTable;
        query.select(Projections.bean(MstOperationTheatreTable.class, mstOperationTheatreTable.ottId, mstOperationTheatreTable.ottName)).from(mstOperationTheatreTable).where(mstOperationTheatreTable.isActive.eq(true)).where(mstOperationTheatreTable.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstOperationTheatreTable.ottName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
