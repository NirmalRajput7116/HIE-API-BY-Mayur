...
@Service
public class MstTimeUnitService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstTimeUnitForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstTimeUnit mstTimeUnit = QMstTimeUnit.mstTimeUnit;
        query.select(Projections.bean(MstTimeUnit.class, mstTimeUnit.tuId, mstTimeUnit.tuName)).from(mstTimeUnit).where(mstTimeUnit.isActive.eq(true)).where(mstTimeUnit.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstTimeUnit.tuName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
