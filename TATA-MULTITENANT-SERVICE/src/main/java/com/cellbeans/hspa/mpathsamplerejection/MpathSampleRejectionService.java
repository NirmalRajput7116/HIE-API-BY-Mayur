...
@Service
public class MpathSampleRejectionService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathSampleRejectionForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathSampleRejection mpathSampleRejection = QMpathSampleRejection.mpathSampleRejection;
        query.select(Projections.bean(MpathSampleRejection.class, mpathSampleRejection.srId, mpathSampleRejection.srReason)).from(mpathSampleRejection).where(mpathSampleRejection.isActive.eq(true)).where(mpathSampleRejection.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathSampleRejection.srReason.contains(globalFilter).or(mpathSampleRejection.srCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
