...
@Service
public class MinvTherapeuticClassService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvTherapeuticClassForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvTherapeuticClass minvTherapeuticClass = QMinvTherapeuticClass.minvTherapeuticClass;
        query.select(Projections.bean(MinvTherapeuticClass.class, minvTherapeuticClass.tcId, minvTherapeuticClass.tcName)).from(minvTherapeuticClass).where(minvTherapeuticClass.isActive.eq(true)).where(minvTherapeuticClass.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvTherapeuticClass.tcName.contains(globalFilter).or(minvTherapeuticClass.tcCode.contains(globalFilter))
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
