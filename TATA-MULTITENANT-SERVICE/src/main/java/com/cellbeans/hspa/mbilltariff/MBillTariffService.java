...
@Service
public class MBillTariffService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getQMbillTariffForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillTariff mbillTariff = QMbillTariff.mbillTariff;
        query.select(Projections.bean(MbillTariff.class, mbillTariff.tariffId, mbillTariff.tariffName)).from(mbillTariff).where(mbillTariff.isActive.eq(true)).where(mbillTariff.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillTariff.tariffName.contains(globalFilter).or(mbillTariff.tariffCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
