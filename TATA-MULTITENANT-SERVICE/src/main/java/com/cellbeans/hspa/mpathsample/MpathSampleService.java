...
@Service
public class MpathSampleService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathSampleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathSample mpathSample = QMpathSample.mpathSample;
        query.select(Projections.bean(MpathSample.class, mpathSample.sampleId, mpathSample.sampleName)).from(mpathSample).where(mpathSample.isActive.eq(true)).where(mpathSample.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathSample.sampleName.contains(globalFilter).or(mpathSample.sampleCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
