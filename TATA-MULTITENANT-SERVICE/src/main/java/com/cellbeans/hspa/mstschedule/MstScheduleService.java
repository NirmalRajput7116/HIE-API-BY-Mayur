...
@Service
public class MstScheduleService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstScheduleForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSchedule mstSchedule = QMstSchedule.mstSchedule;
        query.select(Projections.bean(MstSchedule.class, mstSchedule.scheduleId, mstSchedule.scheduleName)).from(mstSchedule).where(mstSchedule.isActive.eq(true)).where(mstSchedule.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSchedule.scheduleName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
