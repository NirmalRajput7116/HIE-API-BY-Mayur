...
@Service
public class MinvSupplierService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvSupplierForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvSupplier minvSupplier = QMinvSupplier.minvSupplier;
        query.select(Projections.bean(MinvSupplier.class, minvSupplier.supplierId, minvSupplier.supplierName)).from(minvSupplier).where(minvSupplier.isActive.eq(true)).where(minvSupplier.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvSupplier.supplierName.contains(globalFilter).or(minvSupplier.supplierCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
