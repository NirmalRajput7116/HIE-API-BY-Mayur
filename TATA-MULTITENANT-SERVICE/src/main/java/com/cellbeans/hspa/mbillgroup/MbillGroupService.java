...
@Service
public class MbillGroupService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillGroupForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillGroup mbillGroup = QMbillGroup.mbillGroup;
        query.select(Projections.bean(MbillGroup.class, mbillGroup.groupId, mbillGroup.groupName)).from(mbillGroup).where(mbillGroup.isActive.eq(true)).where(mbillGroup.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillGroup.groupName.contains(globalFilter).or(mbillGroup.groupCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
