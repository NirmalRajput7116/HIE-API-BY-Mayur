...
@Service
public class MstAppointmentTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstAppointmentTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstAppointmentType mstAppointmentType = QMstAppointmentType.mstAppointmentType;
        query.select(Projections.bean(MstAppointmentType.class, mstAppointmentType.atId, mstAppointmentType.atName)).from(mstAppointmentType).where(mstAppointmentType.isActive.eq(true)).where(mstAppointmentType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstAppointmentType.atName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
