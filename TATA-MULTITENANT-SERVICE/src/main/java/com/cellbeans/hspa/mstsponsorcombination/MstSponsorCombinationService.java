...
@Service
public class MstSponsorCombinationService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstSponsorCombinationForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstSponsorCombination mstSponsorCombination = QMstSponsorCombination.mstSponsorCombination;
        query.select(Projections.bean(MstSponsorCombination.class, mstSponsorCombination.scId, mstSponsorCombination.scName))
                .from(mstSponsorCombination)
                .where(mstSponsorCombination
                        .isActive.eq(true))
                .where(mstSponsorCombination
                        .isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstSponsorCombination.scName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
