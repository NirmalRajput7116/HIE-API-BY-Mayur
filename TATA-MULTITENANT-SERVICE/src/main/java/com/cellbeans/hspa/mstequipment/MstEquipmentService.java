...
@Service
public class MstEquipmentService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstEquipmentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstEquipment mstEquipment = QMstEquipment.mstEquipment;
        query.select(Projections.bean(MstEquipment.class, mstEquipment.equipmentId, mstEquipment.equipmentName)).from(mstEquipment).where(mstEquipment.isActive.eq(true)).where(mstEquipment.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstEquipment.equipmentName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
