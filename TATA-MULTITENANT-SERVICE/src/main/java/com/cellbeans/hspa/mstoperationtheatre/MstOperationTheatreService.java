...
@Service
public class MstOperationTheatreService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstOperationTheatreForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstOperationTheatre mstOperationTheatre = QMstOperationTheatre.mstOperationTheatre;
        query.select(Projections.bean(MstOperationTheatre.class, mstOperationTheatre.otId, mstOperationTheatre.otName)).from(mstOperationTheatre).where(mstOperationTheatre.isActive.eq(true)).where(mstOperationTheatre.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstOperationTheatre.otName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
