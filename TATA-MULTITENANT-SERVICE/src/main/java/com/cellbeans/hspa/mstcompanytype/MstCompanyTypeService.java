...
@Service
public class MstCompanyTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstCompanyTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstCompanyType mstCompanyType = QMstCompanyType.mstCompanyType;
        query.select(Projections.bean(MstCompanyType.class, mstCompanyType.ctId, mstCompanyType.ctName)).from(mstCompanyType).where(mstCompanyType.isActive.eq(true)).where(mstCompanyType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstCompanyType.ctName.contains(globalFilter).or(mstCompanyType.ctCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
