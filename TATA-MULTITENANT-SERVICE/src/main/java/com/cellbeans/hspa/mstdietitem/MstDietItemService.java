...
@Service
public class MstDietItemService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstDietItemForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstDietItem mstDietItem = QMstDietItem.mstDietItem;
        query.select(Projections.bean(MstDietItem.class, mstDietItem.diId, mstDietItem.diItemName)).from(mstDietItem).where(mstDietItem.isActive.eq(true)).where(mstDietItem.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstDietItem.diItemName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
