...
//    }
    ...
    public List<Tuple> getMbillItemForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillItem mbillItem = QMbillItem.mbillItem;
        query.select(Projections.bean(MbillItem.class, mbillItem.itemId, mbillItem.itemName)).from(mbillItem).where(mbillItem.isActive.eq(true)).where(mbillItem.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillItem.itemName.contains(globalFilter).or(mbillItem.itemCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
