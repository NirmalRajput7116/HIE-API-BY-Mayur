...
@Service
public class MacVoucherTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvItemStorageTypeForDropdown(Integer page, Integer size, String globalFilter) {
        ...
        QMacVoucherType macVoucherType = QMacVoucherType.macVoucherType;
        query.select(Projections.bean(MacVoucherType.class, macVoucherType.vtId, macVoucherType.vtName)).from(macVoucherType).where(macVoucherType.isActive.eq(true)).where(macVoucherType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macVoucherType.vtName.contains(globalFilter).or(macVoucherType.vtCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
