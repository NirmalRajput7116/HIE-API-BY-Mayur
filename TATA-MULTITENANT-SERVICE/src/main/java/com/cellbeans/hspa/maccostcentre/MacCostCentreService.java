...
@Service
public class MacCostCentreService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMacCostCentreForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMacCostCentre macCostCentre = QMacCostCentre.macCostCentre;
        query.select(Projections.bean(MacCostCentre.class, macCostCentre.ccId, macCostCentre.ccName)).from(macCostCentre).where(macCostCentre.isActive.eq(true)).where(macCostCentre.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(macCostCentre.ccName.contains(globalFilter).or(macCostCentre.ccCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
