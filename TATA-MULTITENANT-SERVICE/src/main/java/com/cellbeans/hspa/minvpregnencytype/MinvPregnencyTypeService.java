...
@Service
public class MinvPregnencyTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMinvPregnencyTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMinvPregnencyType minvPregnencyType = QMinvPregnencyType.minvPregnencyType;
        query.select(Projections.bean(MinvPregnencyType.class, minvPregnencyType.ptId, minvPregnencyType.ptName)).from(minvPregnencyType).where(minvPregnencyType.isActive.eq(true)).where(minvPregnencyType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(minvPregnencyType.ptName.contains(globalFilter).or(minvPregnencyType.ptCode.contains(globalFilter))
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
