...
@Service
public class InvCompoundNameService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getInvCompoundNameForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QInvCompoundName invCompoundName = QInvCompoundName.invCompoundName;
        query.select(Projections.bean(InvCompoundName.class, invCompoundName.compoundId, invCompoundName.compoundName)).from(invCompoundName).where(invCompoundName.isActive.eq(true)).where(invCompoundName.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(invCompoundName.compoundName.contains(globalFilter));
            query.orderBy(invCompoundName.compoundName.asc());
        } else {
            query.orderBy(invCompoundName.compoundName.asc());
        }
        query.limit(size);
