...
@Service
public class MemrDiseaseCategoryService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrDiseaseCategoryForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrDiseaseCategory memrDiseaseCategory = QMemrDiseaseCategory.memrDiseaseCategory;
        query.select(Projections.bean(MemrDiseaseCategory.class, memrDiseaseCategory.dcId, memrDiseaseCategory.dcName)).from(memrDiseaseCategory).where(memrDiseaseCategory.isActive.eq(true)).where(memrDiseaseCategory.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrDiseaseCategory.dcName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
