...
@Service
public class MipdAdmissionPurposeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMipdAdmissionPurposeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMipdAdmissionPurpose mipdAdmissionPurpose = QMipdAdmissionPurpose.mipdAdmissionPurpose;
        query.select(Projections.bean(MipdAdmissionPurpose.class, mipdAdmissionPurpose.apId, mipdAdmissionPurpose.apName)).from(mipdAdmissionPurpose).where(mipdAdmissionPurpose.isActive.eq(true)).where(mipdAdmissionPurpose.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mipdAdmissionPurpose.apName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
