...
@Service
public class MstProcedureTypeService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMstProcedureTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstProcedureType mstProcedureType = QMstProcedureType.mstProcedureType;
        query.select(Projections.bean(MstProcedureType.class, mstProcedureType.ptId, mstProcedureType.ptName)).from(mstProcedureType).where(mstProcedureType.isActive.eq(true)).where(mstProcedureType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstProcedureType.ptName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
