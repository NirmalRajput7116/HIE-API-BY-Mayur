...
@Service
public class MemrReferralTemplateService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMemrReferralTemplateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMemrReferralTemplate memrReferralTemplate = QMemrReferralTemplate.memrReferralTemplate;
        query.select(Projections.bean(MemrReferralTemplate.class, memrReferralTemplate.rtId, memrReferralTemplate.rtName)).from(memrReferralTemplate).where(memrReferralTemplate.isActive.eq(true)).where(memrReferralTemplate.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(memrReferralTemplate.rtName.contains(globalFilter));
        }
        query.limit(size);
        query.offset((page * size) - size);
