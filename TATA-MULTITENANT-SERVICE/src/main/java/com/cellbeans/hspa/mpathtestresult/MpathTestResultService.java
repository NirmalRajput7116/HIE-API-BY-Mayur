...
@Service
public class MpathTestResultService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMpathTestResultForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMpathTestResult mpathTestResult = QMpathTestResult.mpathTestResult;
        query.select(Projections.bean(MpathTestResult.class, mpathTestResult.trId, mpathTestResult.trResult)).from(mpathTestResult).where(mpathTestResult.isActive.eq(true)).where(mpathTestResult.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mpathTestResult.trResult.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
