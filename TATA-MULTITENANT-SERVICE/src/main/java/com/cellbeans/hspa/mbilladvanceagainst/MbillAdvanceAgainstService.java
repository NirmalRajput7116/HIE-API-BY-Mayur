...
@Service
public class MbillAdvanceAgainstService {
    @Autowired
...
//    }
    ...
    public List<Tuple> getMbillAdvanceAgainstDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMbillAdvanceAgainst mbillAdvanceAgainst = QMbillAdvanceAgainst.mbillAdvanceAgainst;
        query.select(Projections.bean(MbillAdvanceAgainst.class, mbillAdvanceAgainst.aaId, mbillAdvanceAgainst.aaName)).from(mbillAdvanceAgainst).where(mbillAdvanceAgainst.isActive.eq(true)).where(mbillAdvanceAgainst.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mbillAdvanceAgainst.aaName.contains(globalFilter).or(mbillAdvanceAgainst.aaCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
