...
@Service
public class MstConsentService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstConsentForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstConsent mstConsent = QMstConsent.mstConsent;
        query.select(Projections.bean(MstConsent.class, mstConsent.consentId, mstConsent.consentName)).from(mstConsent).where(mstConsent.isActive.eq(true)).where(mstConsent.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstConsent.consentName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
