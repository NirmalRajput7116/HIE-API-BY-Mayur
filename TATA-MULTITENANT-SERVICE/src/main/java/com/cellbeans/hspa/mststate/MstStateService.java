...
//    }
    ...
    public List<Tuple> getMstStateForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstState mstState = QMstState.mstState;
        query.select(Projections.bean(MstState.class, mstState.stateId, mstState.stateName, mstState.stateCode)).from(mstState).where(mstState.isActive.eq(true)).where(mstState.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstState.stateName.contains(globalFilter).or(mstState.stateCode.contains(globalFilter)));
        }
        query.limit(size);
        query.offset((page * size) - size);
