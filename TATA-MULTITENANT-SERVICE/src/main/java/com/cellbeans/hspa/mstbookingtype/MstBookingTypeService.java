...
@Service
public class MstBookingTypeService {
    @Autowired
...
//        return list;
//    }
    public List<Tuple> getMstBookingTypeForDropdown(Integer page, Integer size, String globalFilter) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstBookingType mstBookingType = QMstBookingType.mstBookingType;
        query.select(Projections.bean(MstBookingType.class, mstBookingType.btId, mstBookingType.btName)).from(mstBookingType).where(mstBookingType.isActive.eq(true)).where(mstBookingType.isDeleted.eq(false));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstBookingType.btName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
