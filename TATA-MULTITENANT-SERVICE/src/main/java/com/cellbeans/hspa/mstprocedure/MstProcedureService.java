...
//    }
    ...
    public List<Tuple> getMstProcedureForDropdown(Integer page, Integer size, String globalFilter, long specialityId) {
        JPAQuery<Tuple> query = new JPAQuery<>(entityManager);
        QMstProcedure mstProcedure = QMstProcedure.mstProcedure;
        query.select(Projections.bean(MstProcedure.class, mstProcedure.procedureId, mstProcedure.procedureName)).from(mstProcedure).where(mstProcedure.isActive.eq(true)).where(mstProcedure.isDeleted.eq(false)).where(mstProcedure.procedureSpeiclityId.specialityId.eq(specialityId));
        if (globalFilter != null && !globalFilter.equals("")) {
            query.where(mstProcedure.procedureName.contains(globalFilter)
            );
        }
        query.limit(size);
        query.offset((page * size) - size);
